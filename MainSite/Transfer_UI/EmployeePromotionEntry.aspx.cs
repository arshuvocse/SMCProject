...
public partial class Transfer_UI_EmployeePromotionEntry : System.Web.UI.Page
{
    ...
    private void GetOneRecord(string idd)
    {
        ...
        if (aDataTable.Rows.Count > 0)
        {
            ...
            try
            {
                if (Convert.ToBoolean(aDataTable.Rows[0]["IsB_DirectlySupervisor"]) == true)
                {
                    //chkIsB_DirectlySupervisor.Checked = true;
                    //chkIsB_DirectlySupervisor_OnCheckedChanged(null, null);
                }
            ...
            try
            {
                if (Convert.ToBoolean(aDataTable.Rows[0]["IsB_ReportingBody"]) == true)
                {
                    chkIsB_ReportingBody.Checked = true;
                    chkIsB_ReportingBody_OnCheckedChanged(null, null);
                }
            }
            catch (Exception)
            {
        ...
        try
        {
            if (Validation())
            {
                ...
                else
                {
                    ...
                    atblEmployeePromotionEntryDAO.IsB_ReportingBody = chkIsB_ReportingBody.Checked;
                    atblEmployeePromotionEntryDAO.IsB_DirectlySupervisor = chkIsB_DirectlySupervisor.Checked;
...
");
    ...
    protected void chkIsB_DirectlySupervisor_OnCheckedChanged(object sender, EventArgs e)
    {
        if (chkIsB_DirectlySupervisor.Checked)
        {
            using (DataTable dt222 = _commonDataLoad.GetEmpDDLIsBoard(companyDropDownList.SelectedValue.ToString()))
            {
