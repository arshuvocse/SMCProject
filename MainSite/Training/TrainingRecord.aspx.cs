...
public partial class Training_TrainingRecord : System.Web.UI.Page
{
 ...
 ButtonVisible();
    ...
    protected void btnAddEmployee_OnClick(object sender, EventArgs e)
    {
        ...
        mpe_1.Show();
        using (DataTable dt = _commonDataLoad.GetCompanyDDLPopUp())
        {
    ...
    {
        pop_ddlDepartment.DataSource = _recordDal.GetDepartmentByCompanyPopUp(Convert.ToInt32(pop_ddlCompany.SelectedValue));
        ...
        using (DataTable dt222 = _commonDataLoad.GetEmpDDLIsActive(pop_ddlCompany.SelectedValue.ToString()))
        {
    ...
    {
        string param = "";
        if (pop_ddlCompany.SelectedIndex > 0)
        {
            param = param + " AND a.CompanyId='" + pop_ddlCompany.SelectedValue + "' ";
        }
        if (pop_ddlDepartment.SelectedIndex > 0)
        {
            param = param + " AND a.DepartmentId='" + pop_ddlDepartment.SelectedValue + "' ";
        }

        if (pop_ddlGrade.SelectedIndex > 0)
        {
            param = param + " AND a.SalaryGradeId ='" + pop_ddlGrade.SelectedValue + "' ";
        }

        if (ddlEmpInfo.SelectedIndex > 0)
        {
            param = param + " AND a.EmpInfoId='" + ddlEmpInfo.SelectedValue + "' ";
        }
        return param;
    }
    protected void pop_btnSearch_OnClick(object sender, EventArgs e)
    {
        ...
        DataTable dt = _recordDal.GetEmployee(Parameter());
...
.Count(r => ((CheckBox)r.FindControl("txt_check")).Checked);
    ...
    private bool ValidateSave()
    {
        ...
        {
            if (txtTotalParticipant.Text != "")
            {
                if (Convert.ToDecimal(txtTotalParticipant.Text) <= 0)
                {
                    isValid = false;
                    aShowMessage.ShowMessageBox(" Total Participant  Required ", this);
                    txtTotalParticipant.Focus();
                }
            }
            else
            {
            ...
            {
                if (Convert.ToDecimal(txtTotalDays.Text) <= 0)
                {
                    isValid = false;
                    aShowMessage.ShowMessageBox(" Total Days  Required ", this);
                    txtTotalDays.Focus();
                }
            }
            else
            ...
            if (txtTotalTrainingHoures.Text != "")
            {
                if (Convert.ToDecimal(txtTotalTrainingHoures.Text) <=0)
                {
                    isValid = false;
                    aShowMessage.ShowMessageBox(" Total Hours  Required ", this);
                    txtTotalTrainingHoures.Focus();
                }
            }
            else
            {
    ...
    protected void txtEndDate_OnTextChanged(object sender, EventArgs e)
    {
        ...
        {
            showMessageBox("Start Date is not correct!! ");
    ...
    public void SaveLog(string id)
    {
        ...
        TrainingRecordMasterAppLogDAO appLogDaoa = new TrainingRecordMasterAppLogDAO();
        {
            ...
            appLogDaoa.TrainingRecordMasterId = mainid;
            appLogDaoa.Comments =txtComment.Text;
            appLogDaoa.CommentsId = commentid;

        };
        int ida = _recordDal.SavAppLog(appLogDaoa);
        ...
        //if (ActionStatus == "Verified")
        {
            ...
            int idb = _recordDal.SavAppLog(appLogDao);
            _recordDal.UpdateJobReqStatus2(ActionStatus, mainid);
        }
    }
    ...
    protected void btn_Save_OnClick(object sender, EventArgs e)
    {
        ...
        SaveSubmit(ApprovalStatus);

    }

    private void SaveSubmit(string acstatus)
    {
        if (ValidateSave())
        {
            ...
            {
                pk = _recordDal.UpdateTrainingRecord(aMaster, Convert.ToInt32(hdpk.Value),
            ...
            if (pk > 0)
            {
                ...
                List<TrainingRecordScheduleDayDAO> aTrainingRecordScheduleDayDaoList =
                    new List<TrainingRecordScheduleDayDAO>();
                for (int i = 0; i < gv_daylist.Rows.Count; i++)
                {
                    CheckBox chkDay = (CheckBox) gv_daylist.Rows[i].FindControl("chkDay");
                    if (chkDay.Checked)
    ...
    //public void SaveAppLog(string id)
    //{
    //    int mainid
    //            = Convert.ToInt32(id);
    //    string ActionStatus = "Verified";
    ...
    public void CountChekcedPerRow()
    {
        ...
        int hour = (int) totalts.TotalHours;
    ...
    private void IscommmText()
    {
        try
        {
            if (chk_Common.Checked)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    string startTime = txt_deadLine.Text.ToString();
                    string EndTime = txtEndTimeComm.Text.ToString();
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        dt.Rows[i]["StartTime"] = startTime;
    ...
    protected void txt_deadLine_Changed(object sender, EventArgs e)
    {
        IscommmText();
    }

    protected void txtEndTimeComm_Changed(object sender, EventArgs e)
    {
        IscommmText();
    }
