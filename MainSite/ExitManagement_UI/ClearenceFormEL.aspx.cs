...
public partial class Survey_ClearenceFormEL : System.Web.UI.Page
{
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    if (chkIsForword.Checked)
                    {
                        ...
                        //
                        aExitDal.UpdateForwardtoOtherEmpExitDetail(Convert.ToInt32(hfEmpInfoId.Value),
                            Convert.ToInt32(Session["EmpInfoIdLat"]), Convert.ToInt32(ddlEmpInfoList.SelectedValue), int.Parse(Request.QueryString["ExitDetailId"]), lblRemarks.Text.Trim());
    ...
    private string EnsurePendingIssuesPrefix(string resource)
    {
        const string prefix = "Pending issues: ";

        if (!chkPendingIssues.Checked || resource.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
        {
            return resource;
        }
    ...
    private bool valida()
    {
        ...
        for (int i = 0; i < itemsDetailGridView.Rows.Count; i++)
        {
            ...
            TextBox resourceTextBox =
                (TextBox) itemsDetailGridView.Rows[i].Cells[0].FindControl("OtherConsumptionTextBox");
    ...
    private List<ClearenceFormDetailDao> PrepareJobLocationDataForSaveDetail()
    {
        ...
        if (itemsDetailGridView.Rows.Count > 0)
        {
            for (int i = 0; i < itemsDetailGridView.Rows.Count; i++)
            {
                ...
                TextBox resourceTextBox = (TextBox)itemsDetailGridView.Rows[i].Cells[0].FindControl("OtherConsumptionTextBox");
                ...
                aLocationDao.Resource = EnsurePendingIssuesPrefix(resourceTextBox.Text);
        ...
        if (valida())
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    aExitDal.UpdateForwardBackEmpExitDetail(Convert.ToInt32(hfEmpInfoId.Value),
