using System;
using System.Collections.Generic;
using System.Data;
using System.IO;
using System.Linq;
using System.Web;
using System.Web.UI;
using System.Web.UI.HtmlControls;
using System.Web.UI.WebControls;
using DAL.Appraisal;
using DAL.COMMON_DAL;
using DAL.HealthCare_DAL;
using DAL.Permission_DAL;
using HELPER_FUNCTIONS.HELPERS;
using DAO.HRIS_DAO;

public partial class HealthCare_UI_HRPanel : System.Web.UI.Page
{
    private CommonDataLoadDAL _commonDataLoad = new CommonDataLoadDAL();
    private KPIInformationViewDAL _aFincDal = new KPIInformationViewDAL();
    ShowMessage aShowMessage = new ShowMessage();
    PermissionDAL aPermissionDal = new PermissionDAL();
    private HRPanelDal aPanelDal = new HRPanelDal();

    private ReimbursmentFormDal formDal = new ReimbursmentFormDal();

    int visibleRowCount = 0;

    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            GetCompany();

            LoadInitialDDL();
            load_HR_Panel();
            // load_HR_Panel();

            //  UserPersmissionValidation();

            //DataTable dt = _aFincDal.GetSelfAppraisalList();
            //DataTable dt = _aFincDal.GetAppraisalByKpiPermission( );

            //gv_JdBoard.DataSource = dt;
            //gv_JdBoard.DataBind();         
        }
        try
        {
            gv_JdBoard.UseAccessibleHeader = true;
            // gv_JdBoard.HeaderRow.TableSection = TableRowSection.TableHeader;
            // gv_JdBoard.FooterRow.TableSection = TableRowSection.TableFooter;
        }
        catch (Exception)
        {

            //throw;
        }

    }


    private void LoadInitialDDL()
    {
        DataTable dtPer = new DataTable();


        try
        {
            dtPer = _commonDataLoad.GetblHCTwoComPermisionHC(" and Md.IsForward=1");
        }
        catch (Exception)
        {

            //throw;
        }
        if (dtPer.Rows.Count > 0)
        {
            using (DataTable dt = _commonDataLoad.GetCompanyAllHC())
            {

                ddlCompany.DataSource = dt;
                ddlCompany.DataValueField = "Value";
                ddlCompany.DataTextField = "TextField";
                ddlCompany.DataBind();
                ddlCompany.SelectedIndex = 1;
                ddlCompany_OnSelectedIndexChanged(null, null);
            }
        }
        else
        {

            using (DataTable dt = _commonDataLoad.GetCompany())
            {

                ddlCompany.DataSource = dt;
                ddlCompany.DataValueField = "Value";
                ddlCompany.DataTextField = "TextField";
                ddlCompany.DataBind();
                ddlCompany.SelectedIndex = 1;
                ddlCompany_OnSelectedIndexChanged(null, null);
            }
        }




    }

    public void GetCompany()
    {
        DataTable dtcomp = aPermissionDal.GetCompany();
        lchk_Company.DataValueField = "CompanyId";
        lchk_Company.DataTextField = "ShortName";
        lchk_Company.DataSource = dtcomp;
        lchk_Company.DataBind();

        DataTable userdata = aPermissionDal.GetUserCompany(Session["UserId"].ToString());
        for (int i = 0; i < userdata.Rows.Count; i++)
        {
            for (int j = 0; j < lchk_Company.Items.Count; j++)
            {
                if (lchk_Company.Items[j].Text.Trim() == userdata.Rows[i]["ShortName"].ToString())
                {
                    lchk_Company.Items[j].Selected = true;

                }
            }
        }
    }

    public void UserPersmissionValidation()
    {
        try
        {
            string filepath = Path.GetDirectoryName(Request.Path);
            filepath = filepath.TrimStart('\\');
            filepath = "../" + filepath + "/" + Path.GetFileName(Request.Path);
            DataTable dtuserpermission = aPermissionDal.GetPermissionForUser(Session["UserId"].ToString(), filepath);
            if (dtuserpermission.Rows.Count > 0)
            {
                if (dtuserpermission.Rows[0]["UserTypeId"].ToString() != "3" ||
                    dtuserpermission.Rows[0]["UserTypeId"].ToString() != "4")
                {
                    ViewState["Add"] = dtuserpermission.Rows[0]["Add"].ToString();
                    ViewState["Edit"] = dtuserpermission.Rows[0]["Edit"].ToString();
                    ViewState["View"] = dtuserpermission.Rows[0]["View"].ToString();
                    ViewState["Delete"] = dtuserpermission.Rows[0]["Delete"].ToString();

                    gv_JdBoard.Columns[gv_JdBoard.Columns.Count - 1].Visible =
                        Convert.ToBoolean(ViewState["View"].ToString());
                    gv_JdBoard.Columns[gv_JdBoard.Columns.Count - 2].Visible =
                        Convert.ToBoolean(ViewState["Delete"].ToString());
                    gv_JdBoard.Columns[gv_JdBoard.Columns.Count - 3].Visible =
                        Convert.ToBoolean(ViewState["Edit"].ToString());
                }
            }
            else
            {
                Response.Redirect("../DashBoard_UI/DashBoard.aspx");
            }
        }
        catch (Exception ex)
        {

            aShowMessage.ShowMessageBox(ex.ToString(), this);
        }
    }

    protected void homeButton_OnClick(object sender, EventArgs e)
    {
        Response.Redirect("../DashBoard_UI/DashBoard.aspx");
    }

    public string CompanyId()
    {
        string companyid = "";
        for (int i = 0; i < lchk_Company.Items.Count; i++)
        {
            if (lchk_Company.Items[i].Selected)
            {
                companyid = companyid + "'" + lchk_Company.Items[i].Value + "'" + ",";
            }
        }

        companyid = companyid.TrimEnd(',');
        return companyid;
    }

    protected void detailsViewButton_OnClick(object sender, EventArgs e)
    {
        Session["Status"] = "Add";
        Response.Redirect("~/Appraisal/AppraisalSelfFunctional.aspx");
    }

    protected void btn_edit_OnClick(object sender, EventArgs e)
    {
        //LinkButton lb = (LinkButton)sender;
        //GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        //int rowID = gvRow.RowIndex;

        //HiddenField EmpInfoId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("EmpInfoId");
        //HiddenField FinancialYearId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("FinancialYearId");

        //HiddenField mastrId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("AppraisalSelfMasterId");
        //Session["Status"] = "Edit";

        //Response.Redirect("AppraisalSelfFunctional.aspx?EmpInfoId=" + EmpInfoId.Value + "&financialYearId=" + FinancialYearId.Value + "");
    }





    protected void btn_eview_OnClick(object sender, EventArgs e)
    {

        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowID = gvRow.RowIndex;
        HiddenField hfMasterId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("hfAppraisalSelfMasterId");
        Label Status = (Label)gv_JdBoard.Rows[rowID].FindControl("ActionStatus");

        string ForEmpId = gv_JdBoard.DataKeys[rowID][0].ToString();

        if (Session["EmpInfoId"] != null)
        {


            Response.Redirect("ExpenseReimbursementFormForHRPayment.aspx?MID=" + hfMasterId.Value.Trim() + "&Mode=HR");

            //if (Session["EmpInfoId"].ToString() == ForEmpId)
            //{
            //    //if (Status.Text.Trim() == "Approved" || Status.Text.Trim() == "Review" || Status.Text.Trim() == "Draft")
            //    //{
            //    //    aShowMessage.ShowMessageBox("Can not Edit", this);
            //    //}
            //    //else
            //    //{
            //    //    Response.Redirect("ExpenseReimbursementFormSelfEntry.aspx?MID=" + hfMasterId.Value.Trim());
            //    //}

            //    Response.Redirect("ExpenseReimbursementFormSelfEntry.aspx?MID=" + hfMasterId.Value.Trim());
            //}
            //else
            //{
            //    aShowMessage.ShowMessageBox("Can not edit", this);
            //}
        }

        //if (dt.Rows.Count > 0)
        //{
        //    if (dt.Rows[0]["ActionStatus"].ToString().Trim() == "Draft" || dt.Rows[0]["ActionStatus"].ToString().Trim() == "Review")
        //    {
        //        Response.Redirect("ExpenseReimbursementFormSelfEntry.aspx?MID=" + hfMasterId.Value.Trim());
        //    }
        //    else
        //    {

        //        AlertMessageBoxShow("You Can not edit this.....");

        //    }
        //}


    }

    protected void btn_Fview_OnClick(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowID = gvRow.RowIndex;
        string MasterId = gv_JdBoard.DataKeys[rowID]["ReimbursFromMasterId"].ToString();
        DataTable DT = aPanelDal.Get_CommitteeFeedback(MasterId);
        if (DT.Rows.Count > 0)
        {
            gv_EmpListSearch.DataSource = DT;
            gv_EmpListSearch.DataBind();

            ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$('#exampleModal23').modal('show')", true);

            // data-toggle="modal" data-target="#exampleModal23"
        }
        else
        {
            aShowMessage.ShowMessageBox("There is no feedback for this Application", this);
        }


        //System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('KPIInformationDetailsView.aspx?EmpInfoId=" + EmpInfoId.Value + "&financialYearId=" + FinancialYearId.Value + "' ,'_blank');", true);
    }


    protected void btn_print_OnClick(object sender, EventArgs e)
    {

        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowID = gvRow.RowIndex;
        Session["dsdadf"] = "";
        Session["dsdadf"] = "View";
        HiddenField MasterId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("hfAppraisalSelfMasterId");
        //  Response.Redirect("../HealthCare_UI/ApplicationView_HR.aspx?MID=" + MasterId.Value.Trim());


        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('ApplicationView_HR.aspx?MID=" + MasterId.Value.Trim() + "' ,'_blank');", true);

        //   string url = "../HealthCare_UI/ApplicationView_HR.aspx?MID=" + MasterId.Value;
        //  string fullURL = "window.open('" + url + "', '_blank', 'height=600,width=900,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=no,titlebar=no' );";
        //  ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_WINDOW", fullURL, true);


        //LinkButton lb = (LinkButton)sender;
        //GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        //int rowID = gvRow.RowIndex;

        //HiddenField EmpInfoId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("EmpInfoId");
        //HiddenField FinancialYearId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("FinancialYearId");

        //HiddenField mastrId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("AppraisalSelfMasterId");


        //string url = "../Report_UI/KPIInformationReportViewer.aspx?EmpInfoId=" + EmpInfoId.Value + "&financialYearId=" + FinancialYearId.Value;
        //string fullURL = "window.open('" + url + "', '_blank', 'height=600,width=900,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=no,titlebar=no' );";
        //ScriptManager.RegisterStartupScript(this, typeof(string), "OPEN_WINDOW", fullURL, true);

        // Response.Redirect("../Report_UI/KPIInformationReportViewer.aspx?EmpInfoId=" + EmpInfoId.Value + "&financialYearId=" + FinancialYearId.Value + "");
    }

    protected void btn_view_OnClick(object sender, EventArgs e)
    {
        //LinkButton lb = (LinkButton)sender;
        //GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        //int rowID = gvRow.RowIndex;

        //HiddenField EmpInfoId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("EmpInfoId");
        //HiddenField FinancialYearId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("FinancialYearId");

        //HiddenField mastrId = (HiddenField)gv_JdBoard.Rows[rowID].FindControl("AppraisalSelfMasterId");
        //Session["Status"] = "Edit";



        //Response.Redirect("KPIInformationDetailsView.aspx?EmpInfoId=" + EmpInfoId.Value + "&financialYearId=" + FinancialYearId.Value + "");
    }

    protected void ddlCompany_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        Session["cid"] = ddlCompany.SelectedValue;

        int comid = Convert.ToInt32(ddlCompany.SelectedValue);
        if (comid == 4)
        {
            comid = 2;
        }
        DataTable dt = _aFincDal.GetFianncialYearByComIdDDl(Convert.ToInt32(comid));
        ddlFinancialYear.DataSource = dt;
        ddlFinancialYear.DataValueField = "Value";
        ddlFinancialYear.DataTextField = "TextField";
        ddlFinancialYear.DataBind();

        using (DataTable dt2 = _commonDataLoad.GetDDLComDepartment(Convert.ToString(comid)))
        {
            ddlDept.DataSource = dt2;
            ddlDept.DataValueField = "Value";
            ddlDept.DataTextField = "TextField";
            ddlDept.DataBind();
        }


    }

    protected void ddlFinancialYear_OnSelectedIndexChanged(object sender, EventArgs e)
    {

        if (ddlActoinStatus.SelectedValue != "Review")
        {

            gridContainer14.Visible = false;
            gridContainer1.Visible = true;

            load_HR_Panel();
        }
    }

    protected void SearchButton_OnClick(object sender, EventArgs e)
    {
        if (ddlActoinStatus.SelectedValue != "Review" && ddlActoinStatus.SelectedValue!= "Rejected")
        {

            gridContainer14.Visible = false;
            gridContainer1.Visible = true;

            load_HR_Panel();
        }
        else
        {
            gv_JdBoard.DataSource = null;
            gv_JdBoard.DataBind();

            GridView1.DataSource = null;
            GridView1.DataBind();
            DataTable Dt = aPanelDal.Get_HRPanel_ReturnApplication(ParameterRejec());
            if (Dt.Rows.Count > 0)
            {

                

                gridContainer14.Visible = true;
                gridContainer1.Visible = false;

                for (int i = 0; i < Dt.Rows.Count; i++)
                {

                    string hfSalaryLoationId = Dt.Rows[i]["SalaryLoationId"].ToString();
                    string hfApplicationType = Dt.Rows[i]["Type"].ToString();

                    DataTable DtCheck = aPanelDal.Get_HRPanelCheckCommetiCompany(hfApplicationType, hfSalaryLoationId, Convert.ToInt32(ddlCompany.SelectedValue));
                    if (DtCheck.Rows.Count == 0)
                    {
                        Dt.Rows.RemoveAt(i);
                        i--;
                    }
                }

                GridView1.DataSource = Dt;
                GridView1.DataBind();

                gv_JdBoard.DataSource = Dt;
                gv_JdBoard.DataBind();


            }

        }
        

    }


    private void load_HR_Panel()
    {
        try
        {

            DataTable DtNormal = new DataTable();
            try
            {
                DtNormal = aPanelDal.Get_HRPanel(Parameter());
            }
            catch
            {

            }

            DataTable DtHolding = new DataTable();
            try
            {

                if (ddlCompany.SelectedValue == "4")
                {
                    //DtHolding = aPanelDal.Get_HRPanelHolding(HoldingParameter());
                    DtHolding = aPanelDal.Get_HRPanelPayroll(ParameterPAyRoll());
                }

            }
            catch
            {

            }

            DataTable Dt = null;

            // Check if DtNormal is not null, then copy it to Dt
            if (DtNormal != null)
            {
                Dt = DtNormal.Copy();
            }
            else
            {
                // If DtNormal is null, initialize an empty DataTable to avoid null reference issues
                Dt = new DataTable();
            }

            // Check if DtHolding is not null before merging
            if (DtHolding != null)
            {
                Dt.Merge(DtHolding);
            }

            if (Dt.Rows.Count > 0)
            {

                gv_JdBoard.DataSource = null;
                gv_JdBoard.DataBind();

                gv_Export.DataSource = null;
                gv_Export.DataBind();

                for (int i = 0; i < Dt.Rows.Count; i++)
                {

                    string HealthcareCompanyId = Dt.Rows[i]["HealthcareCompanyId"].ToString();
                    string hfSalaryLoationId = Dt.Rows[i]["SalaryLoationId"].ToString();
                    string hfApplicationType = Dt.Rows[i]["ApplicationType"].ToString();

                    if (HealthcareCompanyId == "1")
                    {

                    }
                    else
                    {

                        DataTable DtCheck = aPanelDal.Get_HRPanelCheckCommeti(hfApplicationType, hfSalaryLoationId);
                        if (DtCheck.Rows.Count == 0)
                        {
                            Dt.Rows.RemoveAt(i);
                            i--;
                        }
                    }
                }

                gv_JdBoard.DataSource = Dt;
                gv_JdBoard.DataBind();

                gv_Export.DataSource = Dt;
                gv_Export.DataBind();

                for (int i = 0; i < gv_JdBoard.Rows.Count; i++)
                {
                    // Local_count++;
                    HiddenField hfActionStatus = (HiddenField)gv_JdBoard.Rows[i].FindControl("hfActionStatus");
                    // Label Sl = (Label)gv_JdBoard.Rows[i].FindControl("LabelSL");  
                    if (hfActionStatus.Value == "Approved" || hfActionStatus.Value == "Rejected")
                    {
                        LinkButton btn = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_view");
                        if (btn != null)
                        {
                            btn.Visible = false;
                        }
                        LinkButton btnReject = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Reject");
                        if (btnReject != null)
                        {
                            btnReject.Visible = hfActionStatus.Value != "Rejected";
                        }
                        LinkButton btnForwardDoctor = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_ForwardDoctor");
                        LinkButton btnPopUp = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_PopUp");
                        LinkButton btnReturn = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Return");
                        LinkButton btnMd = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_MD");
                        if (hfActionStatus.Value == "Rejected")
                        {
                            if (btnForwardDoctor != null) btnForwardDoctor.Visible = false;
                            if (btnPopUp != null) btnPopUp.Visible = false;
                            if (btnReturn != null) btnReturn.Visible = false;
                            if (btnMd != null) btnMd.Visible = false;
                        }
                    }


                    string HR = gv_JdBoard.DataKeys[i]["HR"].ToString();

                    if (HR != "HR")
                    {
                        LinkButton btn_view = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_view");
                        LinkButton btn_ForwardDoctor = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_ForwardDoctor");
                        LinkButton btn_PopUp = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_PopUp");
                        LinkButton returnb = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Return");
                        LinkButton MD = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_MD");
                        LinkButton reject = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Reject");

                        btn_view.Visible = false;
                        btn_ForwardDoctor.Visible = false;
                        btn_PopUp.Visible = false;
                        returnb.Visible = false;
                        MD.Visible = false;
                        reject.Visible = false;

                    }
                    // Sl.Text = Local_count.ToString();
                }
            }
            else
            {
                gv_JdBoard.DataSource = null;
                gv_JdBoard.DataBind();

                gv_Export.DataSource = null;
                gv_Export.DataBind();
            }

            //gv_JdBoard.UseAccessibleHeader = true;
            //gv_JdBoard.HeaderRow.TableSection = TableRowSection.TableHeader;
            //if (gv_JdBoard.TopPagerRow != null)
            //{
            //    gv_JdBoard.TopPagerRow.TableSection = TableRowSection.TableHeader;
            //}
            //if (gv_JdBoard.BottomPagerRow != null)
            //{
            //    gv_JdBoard.BottomPagerRow.TableSection = TableRowSection.TableFooter;
            //}
        }
        catch (Exception)
        {
            //throw;
        }


        try
        {

            gv_JdBoard.UseAccessibleHeader = true;
            //  gv_JdBoard.HeaderRow.TableSection = TableRowSection.TableHeader;
            //  gv_JdBoard.FooterRow.TableSection = TableRowSection.TableFooter;
        }
        catch (Exception)
        {

            //throw;
        }
    }

    public string Parameter()
    {
        string param = "";

        if (ddlCompany.SelectedValue != "")
        {
            param = param + " AND H.CompanyId='" + ddlCompany.SelectedValue + "' ";
        }


        if (ddlFinancialYear.Items.Count > 0)
        {
            if (ddlFinancialYear.SelectedIndex > 0)
            {
                param = param + " AND H.FinancialYearId='" + ddlFinancialYear.SelectedValue + "' ";
            }
        }

        if (ddlDept.Items.Count > 0)
        {
            if (ddlDept.SelectedIndex > 0)
            {
                param = param + " AND H.DepartmentId='" + ddlDept.SelectedValue + "' ";
            }
        }

        if (ddlActoinStatus.SelectedValue != "")
        {
            param = param + " AND H.ActionStatus='" + ddlActoinStatus.SelectedValue + "' ";
        }

        if (ddlType.SelectedValue != "")
        {
            param = param + " AND H.Type='" + ddlType.SelectedValue + "' ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + App_ToDate.Text.Trim() + "'  ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() == "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + DateTime.Now + "'  ";
        }

        if (App_FromDate.Text.Trim() == "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND  H.SubmitDate  between '" + DateTime.Now + "' and '" + App_ToDate.Text.Trim() + "' ";
        }


        return param;
    }

    public string ParameterRejec()
    {
        string param = "";

        if (ddlCompany.SelectedValue != "")
        {
            param = param + " AND H.CompanyId='" + ddlCompany.SelectedValue + "' ";
        }


        if (ddlFinancialYear.Items.Count > 0)
        {
            if (ddlFinancialYear.SelectedIndex > 0)
            {
                param = param + " AND H.FinancialYearId='" + ddlFinancialYear.SelectedValue + "' ";
            }
        }

        if (ddlDept.Items.Count > 0)
        {
            if (ddlDept.SelectedIndex > 0)
            {
                param = param + " AND H.DepartmentId='" + ddlDept.SelectedValue + "' ";
            }
        }

        if (ddlActoinStatus.SelectedValue != "")
        {
            param = param + " AND tblReimbursementSelfAppLog.ActionStatus='" + ddlActoinStatus.SelectedValue + "' ";
        }

        if (ddlType.SelectedValue != "")
        {
            param = param + " AND H.Type='" + ddlType.SelectedValue + "' ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + App_ToDate.Text.Trim() + "'  ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() == "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + DateTime.Now + "'  ";
        }

        if (App_FromDate.Text.Trim() == "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND  H.SubmitDate  between '" + DateTime.Now + "' and '" + App_ToDate.Text.Trim() + "' ";
        }


        return param;
    }

    public string ParameterPAyRoll()
    {
        string param = "";

        if (ddlCompany.SelectedValue != "")
        {
            param = param + " AND EMP.HealthcareCompanyId='" + ddlCompany.SelectedValue + "' ";
        }


        if (ddlFinancialYear.Items.Count > 0)
        {
            if (ddlFinancialYear.SelectedIndex > 0)
            {
                param = param + " AND H.FinancialYearId='" + ddlFinancialYear.SelectedValue + "' ";
            }
        }

        if (ddlDept.Items.Count > 0)
        {
            if (ddlDept.SelectedIndex > 0)
            {
                param = param + " AND H.DepartmentId='" + ddlDept.SelectedValue + "' ";
            }
        }

        if (ddlActoinStatus.SelectedValue != "")
        {
            param = param + " AND H.ActionStatus='" + ddlActoinStatus.SelectedValue + "' ";
        }

        if (ddlType.SelectedValue != "")
        {
            param = param + " AND H.Type='" + ddlType.SelectedValue + "' ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + App_ToDate.Text.Trim() + "'  ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() == "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + DateTime.Now + "'  ";
        }

        if (App_FromDate.Text.Trim() == "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND  H.SubmitDate  between '" + DateTime.Now + "' and '" + App_ToDate.Text.Trim() + "' ";
        }


        return param;
    }

    public string HoldingParameter()
    {
        string param = "";

        if (ddlCompany.SelectedValue != "")
        {
            param = param + " AND EMP.HealthcareCompanyId='" + ddlCompany.SelectedValue + "' ";
        }


        if (ddlFinancialYear.Items.Count > 0)
        {
            if (ddlFinancialYear.SelectedIndex > 0)
            {
                param = param + " AND FY.FinancialYearDesc='" + ddlFinancialYear.SelectedItem.Text + "' ";
            }
        }

        if (ddlDept.Items.Count > 0)
        {
            if (ddlDept.SelectedIndex > 0)
            {
                param = param + " AND H.DepartmentId='" + ddlDept.SelectedValue + "' ";
            }
        }

        if (ddlActoinStatus.SelectedValue != "")
        {
            param = param + " AND H.ActionStatus='" + ddlActoinStatus.SelectedValue + "' ";
        }

        if (ddlType.SelectedValue != "")
        {
            param = param + " AND H.Type='" + ddlType.SelectedValue + "' ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + App_ToDate.Text.Trim() + "'  ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() == "")
        {
            param = param + " AND H.SubmitDate  between '" + App_FromDate.Text.Trim() + "' and '" + DateTime.Now + "'  ";
        }

        if (App_FromDate.Text.Trim() == "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND  H.SubmitDate  between '" + DateTime.Now + "' and '" + App_ToDate.Text.Trim() + "' ";
        }


        return param;
    }

    public string Parameter2()
    {
        string param = "";
        if (ddlFinancialYear.Items.Count > 0)
        {
            if (ddlFinancialYear.SelectedIndex > 0)
            {
                param = param + " AND a.FinancialYearId='" + ddlFinancialYear.SelectedValue + "' ";
            }
        }

        if (ddlDept.Items.Count > 0)
        {
            if (ddlDept.SelectedIndex > 0)
            {
                param = param + " AND e.DepartmentId='" + ddlDept.SelectedValue + "' ";
            }
        }

        param = param + " AND  tblAppraisalSelfAppLog.Version=CELog.MaxVer ";
        // param = param + " AND  tblAppraisalSelfAppLog.Version=CELog.MaxVer ";
        // param = param + " AND  tblAppraisalSelfAppLog.Version=CELog.MaxVer ";

        return param;

    }

    protected void gv_JdBoard_OnRowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "A")
        {

        }
    }


    public override void VerifyRenderingInServerForm(Control control)
    {
        // //required to avoid the runtime error "  
        //Control 'CheckupGridView1' of type 'GridView' must be placed inside a form tag with runat=server."  
    }

    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
        if (gv_JdBoard.Rows.Count > 0)
        {
            string attachment = "attachment; filename=Employee_Healthcare_Status_List.xls";
            Response.ClearContent();
            Response.AddHeader("content-disposition", attachment);
            Response.ContentType = "application/ms-excel";
            StringWriter sw = new StringWriter();
            HtmlTextWriter htw = new HtmlTextWriter(sw);

            gv_Export.AllowPaging = false;

            //Create a form to contain the grid  
            HtmlForm frm = new HtmlForm();
            gv_Export.Parent.Controls.Add(frm);
            //frm.Attributes["runat"] = "server";
            //frm.Controls.Add(loadGridView);
            //frm.RenderControl(htw);
            gv_Export.HeaderRow.Style.Add("background-color", "#E5EEF1");
            // Set background color of each cell of GridView1 header row
            foreach (TableCell tableCell in gv_Export.HeaderRow.Cells)
            {
                tableCell.Style["background-color"] = "#E5EEF1";
            }
            // Set background color of each cell of each data row of GridView1
            foreach (GridViewRow gridViewRow in gv_Export.Rows)
            {
                gridViewRow.BackColor = System.Drawing.Color.White;
                foreach (TableCell gridViewRowTableCell in gridViewRow.Cells)
                {
                    gridViewRowTableCell.Style["background-color"] = "#FFFFFF";
                }
            }

            string com = "";
            if (ddlCompany.SelectedValue == "1")
            {
                com = "Social Marketing Company";
            }

            if (ddlCompany.SelectedValue == "2")
            {
                com = "SMC Enterprise Ltd.";
            }

            gv_Export.RenderControl(htw);
            string headerTable = @"<span   style='text-align:center'><h3>" + com +
                                 " </h3><h4> Health Care Application Form List</h4>  </span> <span   style='text-align:right'><h5> Print Date: " +
                                 DateTime.Now.ToString("dd-MMM-yyyy") + "</h5></span>";

            HttpContext.Current.Response.Write(headerTable);

            Response.Write(sw.ToString());
            Response.End();

        }
        else
        {
            aShowMessage.ShowMessageBox("No Data Found!!", this);
        }
    }

    protected void gv_DocumentUpload_PreRender(object sender, EventArgs e)
    {
        GridView gv = (GridView)sender;

        if ((gv.ShowHeader == true && gv.Rows.Count > 0)
            || (gv.ShowHeaderWhenEmpty == true))
        {
            //Force GridView to use <thead> instead of <tbody> - 11/03/2013 - MCR.
            gv.HeaderRow.TableSection = TableRowSection.TableHeader;
        }

    }


    protected void btn_MD_OnClick(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowIndex = gvRow.RowIndex;
        HiddenField hffMasterId = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfAppraisalSelfMasterId");
        HiddenField IsMdApproval = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("HFIsMDApproval");
        HiddenField hfActionStatus = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfActionStatus");
        LinkButton btn = (LinkButton)gv_JdBoard.Rows[rowIndex].FindControl("btn_MD");
        hfMasterId.Value = hffMasterId.Value;
        //if (hfActionStatus.Value.Trim() == "Approved")
        {
            if (IsMdApproval.Value == "False" || IsMdApproval.Value == "")
            {

                ScriptManager.RegisterStartupScript(this, this.GetType(), "Popup", "$('#MDModal').modal('show')", true);

            }
            else
            {
                aShowMessage.ShowMessageBox("Already Forwarded to MD Sir", this);
            }
        }
        //else
        //{
        //    aShowMessage.ShowMessageBox("Can Not Forward To MD Sir", this);
        //}


        //LinkButton lb = (LinkButton)sender;
        //GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        //int rowID = gvRow.RowIndex;
        //Session["dsdadf"] = "";
        //Session["dsdadf"] = "View";
        //HiddenField hfMasterId = (HiddenField)loadGridView.Rows[rowID].FindControl("hfReimbursFromMasterId");

    }

    protected void btn_Return_OnClick(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowIndex = gvRow.RowIndex;
        HiddenField hffMasterId = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfAppraisalSelfMasterId");

        DataTable Dt = aPanelDal.Get_MaxLogInformation(hffMasterId.Value);

        if (Dt.Rows.Count > 0)
        {
            //   if (Session["EmpInfoId"].ToString() == Dt.Rows[0]["ForEmpInfoId"].ToString())
            {
                Session["HRReturn"] = "Return";
                Response.Redirect("ExpenseFormMDApproval.aspx?MID=" + hffMasterId.Value + "&PreeEmpId=" + Dt.Rows[0]["ForEmpInfoId"].ToString() + "&LogID=" + Dt.Rows[0]["ReimbursementSelfAppLogId"].ToString() + "");
            }
            //else
            //{
            //    aShowMessage.ShowMessageBox("Can not return Now", this);
            //}
        }
        else
        {
            aShowMessage.ShowMessageBox("Can not return Now", this);
        }

    }

    protected void btn_ForwardDoctor_OnClick(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowIndex = gvRow.RowIndex;
        HiddenField hffMasterId = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfAppraisalSelfMasterId");

        hfDoctorForwardId.Value = hffMasterId.Value;
        txtDoctorComment.Text = string.Empty;
        ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowDoctorModal", "$('#DoctorModal').modal('show')", true);
    }

    protected void btnSubmitDoctor_OnClick(object sender, EventArgs e)
    {
        if (string.IsNullOrEmpty(hfDoctorForwardId.Value))
        {
            aShowMessage.ShowMessageBox("No application selected for forwarding.", this);
            return;
        }

        if (string.IsNullOrWhiteSpace(txtDoctorComment.Text))
        {
            aShowMessage.ShowMessageBox("Please enter comment before forwarding.", this);
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowDoctorModalValidation", "$('#DoctorModal').modal('show')", true);
            return;
        }

        bool Status = formDal.ExpenseReimbursementForwardtoDoctor(hfDoctorForwardId.Value, txtDoctorComment.Text.Trim());
        if (Status)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "HideDoctorModal", "$('#DoctorModal').modal('hide')", true);
            aShowMessage.ShowMessageBox("Forward to doctor Successfully!", this);
            txtDoctorComment.Text = string.Empty;
            hfDoctorForwardId.Value = string.Empty;

            load_HR_Panel();
        }
    }

    protected void btn_Reject_OnClick(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowIndex = gvRow.RowIndex;
        HiddenField hffMasterId = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfAppraisalSelfMasterId");
        HiddenField hfActionStatus = (HiddenField)gv_JdBoard.Rows[rowIndex].FindControl("hfActionStatus");

        if (hfActionStatus != null && hfActionStatus.Value == "Rejected")
        {
            aShowMessage.ShowMessageBox("Application already rejected.", this);
            return;
        }

        hfRejectMasterId.Value = hffMasterId.Value;
        hfRejectForEmpId.Value = gv_JdBoard.DataKeys[rowIndex]["ForEmpInfoId"].ToString();
        txtHRRejectComment.Text = string.Empty;
        ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowHRRejectModal", "$('#HRRejectModal').modal('show')", true);
    }

    protected void btnSubmitHRReject_OnClick(object sender, EventArgs e)
    {
        if (string.IsNullOrEmpty(hfRejectMasterId.Value))
        {
            aShowMessage.ShowMessageBox("No application selected for rejection.", this);
            return;
        }

        if (string.IsNullOrWhiteSpace(txtHRRejectComment.Text))
        {
            aShowMessage.ShowMessageBox("Please enter rejection comment.", this);
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowHRRejectModalValidation", "$('#HRRejectModal').modal('show')", true);
            return;
        }

        int masterId;
        int forEmpId;
        int.TryParse(hfRejectForEmpId.Value, out forEmpId);
        if (!int.TryParse(hfRejectMasterId.Value, out masterId))
        {
            aShowMessage.ShowMessageBox("Invalid application selected.", this);
            return;
        }

        ReimbursementSelfAppLogDAO appLogDao = new ReimbursementSelfAppLogDAO()
        {
            ActionStatus = "Rejected",
            ApproveDate = DateTime.Now,
            ApproveBy = Session["UserId"].ToString(),
            PreEmpInfoId = Convert.ToInt32(Session["EmpInfoId"].ToString()),
            ForEmpInfoId = forEmpId,
            ReimbursFromMasterId = masterId,
            Comments = txtHRRejectComment.Text.Trim(),
            CommentsEMP = Convert.ToInt32(Session["EmpInfoId"].ToString()),
            HRPanel = true,
        };

        int logId = formDal.SaveEmpAppLogForHR(appLogDao);
        bool status = formDal.RejectedUpdateContractural(new ReimbursementSelfAppLogDAO { ReimbursFromMasterId = masterId, ActionStatus = "Rejected" });

        if (logId > 0 && status)
        {
            Response.Redirect("/HealthCare_UI/HRPanel.aspx");
        }
        else
        {
            aShowMessage.ShowMessageBox("Failed to reject application.", this);
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "HideProgressAfterRejectFail", "hideHRRejectModal();", true);
        }
    }

    private void ApplyRejectionUI(int masterId)
    {
        for (int i = 0; i < gv_JdBoard.Rows.Count; i++)
        {
            HiddenField hfMasterRow = (HiddenField)gv_JdBoard.Rows[i].FindControl("hfAppraisalSelfMasterId");
            if (hfMasterRow != null && hfMasterRow.Value == masterId.ToString())
            {
                HiddenField hfActionStatus = (HiddenField)gv_JdBoard.Rows[i].FindControl("hfActionStatus");
                if (hfActionStatus != null)
                {
                    hfActionStatus.Value = "Rejected";
                }

                LinkButton btnForwardDoctor = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_ForwardDoctor");
                LinkButton btnPopUp = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_PopUp");
                LinkButton btnReturn = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Return");
                LinkButton btnMd = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_MD");
                LinkButton btnReject = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_Reject");
                LinkButton btnView = (LinkButton)gv_JdBoard.Rows[i].FindControl("btn_view");

                if (btnForwardDoctor != null) btnForwardDoctor.Visible = false;
                if (btnPopUp != null) btnPopUp.Visible = false;
                if (btnReturn != null) btnReturn.Visible = false;
                if (btnMd != null) btnMd.Visible = false;
                if (btnReject != null) btnReject.Visible = false;
                if (btnView != null) btnView.Visible = false;
                break;
            }
        }
    }


    protected void btnFtoMD_OnClick(object sender, EventArgs e)
    {
        //MDApprovalPermission- This Method also Use in ExpenseFormMDApproval Page  

        if (hfMasterId.Value != "")
        {

            bool status = aPanelDal.MDApprovalPermission(hfMasterId.Value, "1", txtcomments.Text.Trim());
            if (status)
            {
                //btn.Enabled = false;
                //btn.CssClass = "btn btn-sm btnMyDesignOne block";
                aShowMessage.ShowMessageBox("Forwarded To MD Sir", this);

                load_HR_Panel();
            }
        }

    }


    protected void loadGridView_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        gv_JdBoard.PageIndex = e.NewPageIndex;
        this.load_HR_Panel();
    }



    protected void gv_JdBoard_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            if (e.Row.RowState == DataControlRowState.Normal || e.Row.RowState == DataControlRowState.Alternate)
            {
                visibleRowCount++;
                Label labelSL = e.Row.FindControl("LabelSL") as Label;
                if (labelSL != null)
                {
                    labelSL.Text = visibleRowCount.ToString(); // Set the label text to the visible row count
                }
            }
        }
    }
}

