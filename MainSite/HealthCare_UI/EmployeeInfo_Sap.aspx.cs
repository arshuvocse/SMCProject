...
public partial class HealthCare_UI_EmployeeInfo_Sap : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            LoadInitialDDL();
            App_FromDate.Text = DateTime.Now.ToString("dd-MMM-yyyy");
            App_ToDate.Text = DateTime.Now.ToString("dd-MMM-yyyy");
    ...
    private void GETALl()
    {
        DataTable Dt = aDal.Get_EmpInfoAll( StringParameter());
    ...
    private string StringParameter()
    {
        ...
        if (!string.IsNullOrEmpty(ddlEmpInfoNew.SelectedValue))
        {
            param += " and ( e.Pernr = '" + ddlEmpInfoNew.SelectedValue + "' )";
            
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND convert(date,e.ServerDateTime)  between '" + App_FromDate.Text.Trim() + "' and '" + App_ToDate.Text.Trim() + "'  ";
        }

        if (App_FromDate.Text.Trim() != "" && App_ToDate.Text.Trim() == "")
        {
            param = param + " AND  convert(date,e.ServerDateTime)  between '" + App_FromDate.Text.Trim() + "' and '" + DateTime.Now + "'  ";
        }

        if (App_FromDate.Text.Trim() == "" && App_ToDate.Text.Trim() != "")
        {
            param = param + " AND   convert(date,e.ServerDateTime)  between '" + DateTime.Now + "' and '" + App_ToDate.Text.Trim() + "' ";
        }
    ...
    private void LoadInitialDDL()
    {
        ...
        using (DataTable dt = _commonDataLoad.GetEMPSAPDDL())
        {

            ddlEmpInfoNew.DataSource = dt;
            ddlEmpInfoNew.DataValueField = "Value";
            ddlEmpInfoNew.DataTextField = "TextField";
            ddlEmpInfoNew.DataBind(); 
        }
    ...
    protected void btn_eview_OnClick(object sender, EventArgs e)
    {
        ...
        string EmpInfoId = gv_JdBoard.DataKeys[rowID]["EmpId"].ToString();

        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('EmpDetails_Sap.aspx?EmpInfoId=" + EmpInfoId + "' ,'_blank');", true);
    }
    ...
    //        DataTable dt = _aFincDal.GetAppraisalByKpiPermission_New__(ddlCompany.SelectedValue.ToString(), Parameter(), " and  tblBSCAppraisalSelfAppLog.Version=CELog.MaxVer " + Parameter_JJJ(), Parameter(), " and  tblBSCAppraisalSelfAppLog.Version=CELog.MaxVer " + Parameter_JJJ(),"", "");
    ...
    //        else
    //        {
    //            DataTable dt3 = _aFincDal.GetAppraisalByKpiPermission_New__(ddlCompany.SelectedValue.ToString(), Parameter2(), " and  tblBSCAppraisalSelfAppLog.Version=CELog.MaxVer " + Parameter_JJJ(), Parameter2(), " and  tblBSCAppraisalSelfAppLog.Version=CELog.MaxVer " + Parameter_JJJ(),"", "");
