...
public partial class HealthCare_UI_ApplicationWaitList : System.Web.UI.Page
{
    ...
    protected void SearchButton_OnClick(object sender, EventArgs e)
    {
        ...
        {
            DataTable dt = approvalPanel.Get_ApplicationWaitingList(Parameter());
            if (dt.Rows.Count > 0)
            {
                ...
                using (DataTable dtemp = _commonDataLoad.GetEmpDDLAActive(ddlCompany.SelectedValue.ToString()))
                {
                    ...
                    {
                        DropDownList ddlForwordEmp = (DropDownList)gv_JdBoard.Rows[i].FindControl("ddlForwordEmp");
    ...
    public string Parameter()
    {
        ...
        {
            param = param + " AND E.EmpInfoId ='" + ddlEmployee.SelectedValue + "' ";
        }
    ...
    protected void btn_Return_OnClick(object sender, EventArgs e)
    {
        ...
        string hffMasterId = gv_JdBoard.DataKeys[rowIndex]["ReimbursFromMasterId"].ToString();
   
        DropDownList aList = (DropDownList)gv_JdBoard.Rows[rowIndex].FindControl("ddlForwordEmp");

        CheckBox aBox = (CheckBox)gv_JdBoard.Rows[rowIndex].FindControl("IsCheck");

        if (aList.SelectedValue != "")
        {
            bool status = approvalPanel.Update_Reimbursment_Log(aList.SelectedValue, hffMasterId, aBox.Checked);
            ...
            if (status)
            {
                ...
                aShowMessage.ShowMessageBox("Succesfully Updated", this);
            }
        }
        else
        {
            aShowMessage.ShowMessageBox("Select Forward Emp", this);
        }  
    }
}