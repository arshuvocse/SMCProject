...
public partial class MPBudget_MPBudgetList : System.Web.UI.Page
{
    ...
    private void LoadManpowerBudgetInfo()
    {
        ...
        if (ddlCompany.SelectedValue != "")
        {
            if (ddlDepartment.SelectedValue != "")
            {
                ...
                {
                    DataTable aTable = _commonDataLoad.GetManpowerBudgetListInfo(ddlCompany.SelectedValue, ddlDepartment.SelectedValue, ddlFinYear.SelectedValue, "  And bm.CompanyId IN (" + CompanyId() + ") AND bm.IsActive=1 AND bm.EntryBy=" + _userId);
    ...
    protected void ddlCompany_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlCompany.SelectedIndex > 0)
        {
            ...
            using (DataTable dt = _commonDataLoad.GetDDLFinYearByCompanyIdAll(int.Parse(ddlCompany.SelectedValue)))
            {
    ...
    private bool CheckAchievementsAllocateOrNot(int MainID)
    {
        ...
        using (var db = new HRIS_SMCEntities())
        {
            result = (from t in db.tblMPBudgetMasters
                      where t.MPBudgetMasterId == MainID && t.ActionStatus != "Drafted"
                      select t).Count();
    ...
    protected void loadGridView_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "EditData")
        {
            ...
            if (datKey != null)
            {
                ...
                int mainID = Convert.ToInt32(datKey["MPBudgetMasterId"]);

                if (!CheckAchievementsAllocateOrNot(mainID))
                {

                    Session["Status"] = "Edit";
                    Response.Redirect("MPBudgetEntry.aspx?mid=" + mainID.ToString());
                }
                else
                {
          ...
          if (e.CommandName == "ViewData")
        {
            ...
            if (datKey != null)
            {
               ...
               // string did = datKey["MPBudgetDetailsId"].ToString();
                Session["Status"] = "View";
                Response.Redirect("MPBudgetViewList.aspx?mid=" + loadGridView.DataKeys[rowindex][0].ToString());// + "&mdid=" + loadGridView.DataKeys[rowindex][1].ToString());
            }
        }
    ...
    private void PopUp(Int32 MPBudgetMasterId)
    {
        string url = "../Report_UI/MPBudgetListReportViwer.aspx?rptType=" + MPBudgetMasterId;
