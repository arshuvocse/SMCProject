...
public partial class MasterSetup_UI_EducationResultEntry : System.Web.UI.Page
{
    ValidationDeleteCommonDAL aValidationDeleteCommonDAL = new ValidationDeleteCommonDAL();


    EducationResultEntryDAL aEntryDaL = new EducationResultEntryDAL();
    ...
    public void ButtonVisible()
    {
        ...
        else
        {
            ...
            Response.Redirect("EducationResultView.aspx");
        }
    ...
    private void GetOneRecord(string id)
    {
        ...
        {
            IdHiddenField.Value = dataTable.Rows[rowIndex].Field<Int32>("EducationResultID").ToString(CultureInfo.InvariantCulture);
                ...
                try
                {
                    ...
                    if (areaId > 0)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(),
                          "alert",
                          "alert('Data Saved Successfully...');window.location ='EducationResultView.aspx';",
                          true);
                    }
                }
                catch (Exception)
    ...
    private bool Updateformation(EducationResultEntryDAO prepareDataForUpdate)
    {
        bool retVal;
        try
        {
    ...
    private EducationResultEntryDAO PrepareDataForUpdate()
    {
        var aEntryDao = new EducationResultEntryDAO();

        aEntryDao.EducationResultID = Convert.ToInt32(IdHiddenField.Value);
    ...
    private EducationResultEntryDAO PrepareDataForSave()
    {
        var EntryDao = new EducationResultEntryDAO();
    ...
    protected void editButton_OnClick(object sender, EventArgs e)
    {
        if ( IdHiddenField.Value != "")
        {
            try
            {
                ...
                if (!CheckEducationResultAllocateOrNot(NameTextBox.Text.Trim()))
                    {
                ...
                if (area)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(),
                          "alert",
                          "alert('Data Updated Successfully...');window.location ='EducationResultView.aspx';",
                          true);
                }
                    ...
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(),
                            "alert",
                            "alert('Can not be Updated! Already Defined in Employee Information  Education Result...');window.location ='EducationResultView.aspx';",
                            true);
    ...
    protected void delButton_OnClick(object sender, EventArgs e)
    {
        ...
        if (aEntryDaL.DeleteEntryfoById( IdHiddenField.Value))
        {
            ...
            if (departmentId > 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(),
                    "alert",
                    "alert('Operation Successfull Done...');window.location ='EducationResultView.aspx';",
                    true);
            }
                    ...
                    else
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(),
                            "alert",
                            "alert('Can not be Deleted! Already Defined in Employee Information Education Result...');window.location ='EducationResultView.aspx';",
                            true);
    ...
    private bool CheckEducationResultAllocateOrNot(string Fieldofspecialization)
    {
        bool status = false;

        DataTable dataTable = aValidationDeleteCommonDAL.EducationResultAllocatedOrNotEMP(Fieldofspecialization);
    ...
    private EducationResultEntryDAO PrepareDataForSaveDEL()
    {
        var EntryDao = new EducationResultEntryDAO();
        ...
        EntryDao.Description = NameTextBox.Text.Trim();
        EntryDao.Code = CodeTextBox.Text.Trim();
        EntryDao.EducationResultID = Convert.ToInt32(IdHiddenField.Value);
