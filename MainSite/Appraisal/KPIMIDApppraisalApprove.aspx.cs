...
public partial class Appraisal_KPIMIDApppraisalApprove : System.Web.UI.Page
{
                ...
                //else
                {
                    ...
                    if (DTSupp.Rows.Count>0)
                    {
                        ...
                        for (int i = 0; i < gv_AppraisalBoard.Rows.Count; i++)
                        {
                            ...
                            //if (dt2.Rows.Count == 0)
                            //{
                            //    ((LinkButton)gv_AppraisalBoard.Rows[i].FindControl("btn_View")).Visible = false;
    ...
    protected void txtMidStatus_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DropDownList ddlMidStatus = (DropDownList)gv_AppraisalFuncSUP.Rows[rowIndex].FindControl("txtMidStatus");
    ...
    //saad
    protected void gv_AppraisalFuncSUP_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            string midYearStatus = "";
            object obj = DataBinder.Eval(e.Row.DataItem, "MidYearStatus");

            if (obj != null)
                midYearStatus = obj.ToString().Trim();

            if (!string.IsNullOrEmpty(midYearStatus))
            {
                ListItem item = null;

                // 1️⃣ Try match by VALUE
                item = ddlStatus.Items.FindByValue(midYearStatus);

                // 2️⃣ Try match by TEXT (Behind / On Track)
                if (item == null)
                    item = ddlStatus.Items.FindByText(midYearStatus);

                // 3️⃣ Manual mapping (IMPORTANT)
                if (item == null)
                {
                    if (midYearStatus.Equals("Behind", StringComparison.OrdinalIgnoreCase))
                        item = ddlStatus.Items.FindByValue("Not Completed");
                    else if (midYearStatus.Equals("On Track", StringComparison.OrdinalIgnoreCase))
                        item = ddlStatus.Items.FindByValue("Ongoing");
                }
    ...
    //        if (ddlStatus != null)
    //        {
    //            // Get MidYearStatus value from data
    //            string midYearStatus = string.Empty;
    //            object midYearStatusObj = DataBinder.Eval(e.Row.DataItem, "MidYearStatus");

    //            if (midYearStatusObj != null)
    //            {
    //                midYearStatus = midYearStatusObj.ToString();
    //            }

    //            // Set dropdown value if data exists
    //            if (!string.IsNullOrEmpty(midYearStatus))
    //            {
    //                ListItem item = ddlStatus.Items.FindByValue(midYearStatus);
    //                if (item != null)
    //                {
    //                    ddlStatus.SelectedValue = midYearStatus;
    //                }
    ...
    private void ApplyMidYearStatusColor(DropDownList ddl)
    {
        ...
        switch (selectedValue)
        {
            case "Not Started":
            case "Not Completed":
                ddl.CssClass = ddl.CssClass + " status-red";
                break;
            case "Ongoing":
            case "Completed":
                ddl.CssClass = ddl.CssClass + " status-green";
                break;
            case "Not Relevant":
                ddl.CssClass = ddl.CssClass + " status-yellow";
                break;
            default:
                // Keep only base classes for "Select One.." or empty
                break;
        }
    }
    ...
    private void CalculateBFuncSUPNew()
    {
        decimal MarkTotal = 0; 
...
");
                                ...
                                ScriptManager.RegisterStartupScript(this, this.GetType(),
                                    "alert",
                                    "alert('Operation Successfully...');window.location ='KPIMIDApppraisalApprove.aspx';",
                                    true);
...
");
    ...
    protected void btn_Save1_OnClick(object sender, EventArgs e)
    {
        ...
        if (confirmstatus)
        {
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(),
                       "alert",
                       "alert('Operation Successful...');window.location ='KPIMIDApppraisalApprove.aspx';",
                       true);
        }
        else
        {
    ...
    protected void btnAppraisalFuncSUPSave_OnClick(object sender, EventArgs e)
    {
        if (ValidationFunCSUp() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalFuncSUP.Rows.Count; i++)
            {
                ...
                TextBox txtResult = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtResult");
                TextBox txtMark = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtMark");
                TextBox txtComment = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtComment");
...
//                        throw;
                    ...
                    area.SupervisorMark =   string.IsNullOrWhiteSpace(txtMark.Text) ? (decimal?)null : Convert.ToDecimal(txtMark.Text.Trim());
                    area.MidYearStatus = txtMidStatus.SelectedItem.Text.Trim().ToString();
                    area.IsActive = isActiveCheckBox.Checked;
                    area.Comments = txtComment == null ? "" : txtComment.Text.Trim();
