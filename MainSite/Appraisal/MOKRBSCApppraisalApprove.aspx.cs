...
public partial class Appraisal_MOKRBSCApppraisalApprove : System.Web.UI.Page
{
    ...
    protected void PartA_OnClick(object sender, EventArgs e)
    {
      ...
      hfFinancialYearId.Value = int.Parse(FinancialYearId.Value).ToString();
        ...
        {
            EnsureCommentColumns(dtw2);
            MergeSelfComments(dtw2, dtw);
            ...
            try
            {
                ...
                for (int i = 0; i < dtw2.Rows.Count; i++)
                {
                    ...
                    try
                    {
                        EnsureMidYearStatusItems(txtMidStatus);
                        txtMidStatus.SelectedValue = MapMidYearStatusValue(dtw2.Rows[i]["MidYearStatus"].ToString());
                    }
                    catch (Exception)
                    {
                    ...
                    ApplyMidYearStatusColor(txtMidStatus);
                    if (txtMidStatus.SelectedValue == "Not Started" || txtMidStatus.SelectedValue == "Not Relevant")
                    {
        ...
        else
        {
            ...
            try
            {
                for (int i = 0; i < dtw.Rows.Count; i++)
                {
                    ...
                    {
                        EnsureMidYearStatusItems(txtMidStatus);
                        txtMidStatus.SelectedValue = MapMidYearStatusValue(dtw.Rows[i]["MidYearStatus"].ToString());
                    }
                    catch (Exception)
                    {
    ...
    public void CheckImmmiedietSuperVisor()
    {
        ...
        else  if (Session["EmpInfoId"].ToString() != id_Empid.Value)
        {
            ...
            if (dtempdata.Rows[0]["ReportingEmpId"].ToString() != Session["EmpInfoId"].ToString())
            {
                ...
                {
                    TextBox txtCommentsFunc = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtCommentsFunc");
                    TextBox txtSupervisorComment = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtSupervisorComment");
                    DropDownList txtMidStatus = (DropDownList)gv_AppraisalFuncSUP.Rows[i].FindControl("txtMidStatus");
                    txtCommentsFunc.ReadOnly = true;
                    if (txtSupervisorComment != null)
                    {
                        txtSupervisorComment.ReadOnly = true;
                    }
                }
                //btnAppraisalFuncSUPSave.Visible = true;
                //for (int i = 0; i < gv_AppraisalFuncSUP.Rows.Count; i++)
            ...
            else
            {
                for (int i = 0; i < gv_AppraisalFuncSUP.Rows.Count; i++)
                {
                    ...
                    TextBox txtSupervisorComment = (TextBox)gv_AppraisalFuncSUP.Rows[i].FindControl("txtSupervisorComment");
                    DropDownList txtMidStatus = (DropDownList)gv_AppraisalFuncSUP.Rows[i].FindControl("txtMidStatus");
                    bool disableSupervisorComment = false;
                    txtCommentsFunc.ReadOnly = true;
                    if (txtSupervisorComment != null)
                    {
                        txtSupervisorComment.ReadOnly = disableSupervisorComment;
                    }
                }
            }
        }
    ...
    private static string MapMidYearStatusValue(string status)
    {
        if (string.Equals(status, "Behind", StringComparison.OrdinalIgnoreCase))
            return "Not Completed";
        if (string.Equals(status, "On Track", StringComparison.OrdinalIgnoreCase))
            return "Ongoing";
        return status;
    }
    private static void ApplyMidYearStatusColor(DropDownList ddl)
    {
        if (ddl == null) return;
    ...
    private static bool IsMidYearNotStarted(DropDownList ddl)
    {
        ...
        return value == "Not Started" || text == "Not Started";
    }
    private static void EnsureCommentColumns(DataTable dt)
    {
        if (dt == null) return;

        if (!dt.Columns.Contains("CommentsFunc"))
        {
            dt.Columns.Add(new DataColumn("CommentsFunc", typeof(string)));
        }
        if (!dt.Columns.Contains("SupervisorComments"))
        {
            dt.Columns.Add(new DataColumn("SupervisorComments", typeof(string)));
        }
    }
    private static void MergeSelfComments(DataTable supervisorTable, DataTable selfTable)
    {
        if (supervisorTable == null || selfTable == null) return;
        if (!supervisorTable.Columns.Contains("AppraisalSelfFucAreaId") || !selfTable.Columns.Contains("AppraisalSelfFucAreaId")) return;
        if (!supervisorTable.Columns.Contains("CommentsFunc") || !selfTable.Columns.Contains("CommentsFunc")) return;

        Dictionary<int, string> selfComments = new Dictionary<int, string>();
        foreach (DataRow row in selfTable.Rows)
        {
            int id;
            if (!int.TryParse(Convert.ToString(row["AppraisalSelfFucAreaId"]), out id)) continue;

            string comment = Convert.ToString(row["CommentsFunc"]);
            if (!selfComments.ContainsKey(id))
            {
                selfComments.Add(id, comment);
            }
        }

        foreach (DataRow row in supervisorTable.Rows)
        {
            int id;
            if (!int.TryParse(Convert.ToString(row["AppraisalSelfFucAreaId"]), out id)) continue;

            string current = Convert.ToString(row["CommentsFunc"]);
            if (!string.IsNullOrWhiteSpace(current)) continue;

            string comment;
            if (selfComments.TryGetValue(id, out comment))
            {
                row["CommentsFunc"] = comment;
            }
        }
    }
...
");
                                ...
                                ScriptManager.RegisterStartupScript(this, this.GetType(),
                                    "alert",
                                    "alert('Operation Successfully...');window.location ='MOKRBSCApppraisalApprove.aspx';",
                                    true);
...
");
    ...
    protected void btn_Save1_OnClick(object sender, EventArgs e)
    {
        ...
        if (confirmstatus)
        {
            ...
            ScriptManager.RegisterStartupScript(this, this.GetType(),
                       "alert",
                       "alert('Operation Successful...');window.location ='MOKRBSCApppraisalApprove.aspx';",
                       true);
        }
        else
        {
    ...
    protected void isActiveCheckBox_OnCheckedChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            if (Convert.ToDecimal(((TextBox)gv_AppraisalFuncSUP.Rows[rowIndex].Cells[4].FindControl("txtWeight")).Text) >=
           Convert.ToDecimal(((TextBox)gv_AppraisalFuncSUP.Rows[rowIndex].Cells[11].FindControl("txtselfMark")).Text))
            {
            ...
            {
                (((TextBox)gv_AppraisalFuncSUP.Rows[rowIndex].Cells[11].FindControl("txtselfMark")).Text) = 0.ToString();
                AlertMessageBoxShow("Self-Mark must be less then or Equal to Weight (Number)");
            }
        }
        catch (Exception)
        {
            ((TextBox)gv_AppraisalFuncSUP.Rows[rowIndex].Cells[4].FindControl("txtWeight")).Text = "0";
    ...
    protected void SupervisorMark_OnTextChanged(object sender, EventArgs e)
    {
        ...
        TextBox txtselfMark = (TextBox)gv_AppraisalFuncSUP.Rows[rowIndex].FindControl("txtselfMark");
        ...
        if (ValidationFunCSUp() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalFuncSUP.Rows.Count; i++)
            {
                ...
                if (hdnDimensionStr.Value != "" )
                {
                    ...
                    area.CommentsFunc = txtCommentsFunc == null ? "" : txtCommentsFunc.Text.Trim();
                    area.SupervisorComments = txtSupervisorComment == null ? "" : txtSupervisorComment.Text.Trim();
                   // area.KpiWeight = Convert.ToDecimal(txtKpiWeight.Text.Trim().ToString());
                    ...
                    area.KpiWeight = Convert.ToDecimal(txtWeight.Text.Trim().ToString());
                    area.SupervisorMark =
                        string.IsNullOrEmpty(txtSuperMark.Text) ? (decimal?)null : Convert.ToDecimal(txtSuperMark.Text.Trim());
                    //Convert.ToDecimal(txtSuperMark.Text.Trim().ToString());
                  //  string.IsNullOrEmpty(txtSuperMark.Text) ? 0 : Convert.ToDecimal(txtSuperMark.Text.Trim().ToString());
                    area.MidYearStatus = txtMidStatus.SelectedValue.Trim().ToString();
    ...
    protected void SupervisorScore_OnTextChanged(object sender, EventArgs e)
    {
        ...
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowID = gvRow.RowIndex;
        TextBox SelfScore = (TextBox)gv_AppraisalPartBSUP.Rows[rowID].FindControl("SelfScore");
        Label SetScore = (Label)gv_AppraisalPartBSUP.Rows[rowID].FindControl("SetScore");
        TextBox txtsupWeight = (TextBox)gv_AppraisalPartBSUP.Rows[rowID].FindControl("SupervisorScore");
        decimal wet = 0, supervisor = 0;
        try
        {
            wet = Convert.ToDecimal(SetScore.Text);
        }
        catch
        {
        ...
        if (supervisor > wet)
        {
            txtsupWeight.Text = wet.ToString();
            AlertMessageBoxShow("Supervisor Mark Cannot Be Greater Then  Mark");
        }
        ...
        if (ValidationBehaveSupp() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalPartBSUP.Rows.Count; i++)
            {
                ...
                if (txtSkillInfo.Text.Trim().ToString() != "" && supervisorScore.Text.Trim().ToString() != "")
                {
                    ...
                    area.BSCAppraisalSelfBehaveId = Convert.ToInt32(gv_AppraisalPartBSUP.DataKeys[i][0].ToString());
                    aList.Add(area);
