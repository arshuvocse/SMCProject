...
public partial class Appraisal_MOKRBSCAppraisalDashboardSelf : System.Web.UI.Page
{
    private CommonDataLoadDAL _commonDataLoad = new CommonDataLoadDAL();
    private HRIS_SMCEntities _hrEntities = new HRIS_SMCEntities();
    private MBSCAppraisalFunctionalPartDAL _appPartA = new MBSCAppraisalFunctionalPartDAL();
    private MBSCOKRAppraislDashboardDAL _appDashboard = new MBSCOKRAppraislDashboardDAL();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            radOp.Items.FindByText("Supervisor").Enabled = false;
            radOp.Items.FindByText("Supervisor").Attributes.CssStyle.Add("display", "none");
            LoadInitialDDL();
    ...
    protected void PartAOwn_OnClick(object sender, EventArgs e)
    {
        ...
        DataTable dt = _appPartA.GetAppraisalSelf(Convert.ToInt32(selfMaster));
        DataTable dtw = _appPartA.GetAppraisalSelfDetailsLatest(Convert.ToInt32(selfMaster));


        DataTable dtw2 = _appPartA.GetAppraisalfDetailsFromSupLatest(Convert.ToInt32(selfMaster));

        if (dtw.Rows.Count != dtw2.Rows.Count)
        {
            dtw2 = dtw;
        }
        ...
        catch (Exception)
        {
            ...
            DropDownList txtMidStatus = (DropDownList)gv_AppraisalFunc.Rows[i].FindControl("txtMidStatus");
                ...
                for (int j = 0; j < txtMidStatus.Items.Count; j++)
                {
                    ...
                    if(gv_AppraisalFunc.DataKeys[i][0].ToString() == "Not Started")
                {
                    txtselfMark.ReadOnly = true;
                    txtselfMark.Text = "";
                }
                    if (gv_AppraisalFunc.DataKeys[i][0].ToString() == "Not Relevant")
                    {
                        txtselfMark.ReadOnly = true;
                        txtselfMark.Text = "";
                }
    ...
    protected void gv_AppraisalFunc_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType != DataControlRowType.DataRow) return;

        DropDownList ddl = (DropDownList)e.Row.FindControl("txtMidStatus");
        if (ddl == null) return;

        EnsureMidYearStatusItems(ddl);

        string mid = Convert.ToString(DataBinder.Eval(e.Row.DataItem, "MidYearStatus"));
        mid = (mid == null) ? "" : mid.Trim();

        if (mid != "")
        {
            ListItem li = ddl.Items.FindByValue(mid); // Value match
            if (li == null)
                li = ddl.Items.FindByText(mid); // Text match (On Track / Behind)

            // Mapping (DB text -> dropdown value)
            if (li == null)
            {
                if (mid.Equals("Behind", StringComparison.OrdinalIgnoreCase))
                    li = ddl.Items.FindByValue("Not Completed");
                else if (mid.Equals("On Track", StringComparison.OrdinalIgnoreCase))
                    li = ddl.Items.FindByValue("Ongoing");
            }
        ...
        ApplyMidYearStatusColor(ddl);
        SetMidYearInputsState(e.Row, IsMidYearMarkDisabled(ddl), false);
    }

    private static void EnsureMidYearStatusItems(DropDownList ddl)
    {
        ddl.Items.Clear();
        ddl.Items.Add(new ListItem("Select One..", "0"));
        ddl.Items.Add(new ListItem("Not Started", "Not Started"));
        ddl.Items.Add(new ListItem("On Track", "Ongoing"));
        ddl.Items.Add(new ListItem("Completed", "Completed"));
        ddl.Items.Add(new ListItem("Behind", "Not Completed"));
        ddl.Items.Add(new ListItem("Not Relevant", "Not Relevant"));
    }

    private void ApplyMidYearStatusColor(DropDownList ddl)
    {
        if (ddl == null) return;

        string v = (ddl.SelectedValue == null) ? "" : ddl.SelectedValue.Trim();
        string t = (ddl.SelectedItem == null || ddl.SelectedItem.Text == null) ? "" : ddl.SelectedItem.Text.Trim();

        string cls = ddl.CssClass ?? "";
        cls = Regex.Replace(cls, @"\bstatus-(red|green|yellow)\b", "").Trim();

        if (v == "Not Started" || v == "Not Completed" || t == "Not Started" || t == "Behind" || t == "Not Completed")
            cls = (cls + " status-red").Trim();
        else if (v == "Ongoing" || v == "Completed" || t == "On Track" || t == "Ongoing" || t == "Completed")
            cls = (cls + " status-green").Trim();
        else if (v == "Not Relevant" || t == "Not Relevant")
            cls = (cls + " status-yellow").Trim();

        ddl.CssClass = cls;
    }

    private static bool IsMidYearMarkDisabled(DropDownList ddl)
    {
        if (ddl == null) return false;

        string value = (ddl.SelectedValue ?? "").Trim();
        string text = (ddl.SelectedItem == null || ddl.SelectedItem.Text == null)
            ? ""
            : ddl.SelectedItem.Text.Trim();

        return value == "Not Started" || text == "Not Started" || value == "Not Relevant" || text == "Not Relevant";
    }

    private static void SetMidYearInputsState(GridViewRow row, bool disable, bool clearMark)
    {
        if (row == null) return;

        TextBox txtselfMark = (TextBox)row.FindControl("txtselfMark");
        if (txtselfMark != null)
        {
            txtselfMark.ReadOnly = disable;
            if (disable && clearMark)
            {
                txtselfMark.Text = "";
            }
        }
    ...
    private void ModifyGridViewRows()
    {
        // Loop through all rows of GridView
        for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
        {
            GridViewRow row = gv_AppraisalFunc.Rows[i];

            // Find controls (Labels)
            Label txtObjectiveGoal = (Label)row.FindControl("txtObjectiveGoal");
            Label txtDimensionStr = (Label)row.FindControl("txtDimensionStr");

            // Safely retrieve the values, ensuring no null values cause errors
            string objectiveGoal = DataBinder.Eval(row.DataItem, "ObjectiveGoal") as string;
            string dimensionStr = DataBinder.Eval(row.DataItem, "DimensionStr") as string;

            // Check if this is the first row
            if (i == 0)
            {
                // Set text values for the first row, using fallback values if data is null
                txtObjectiveGoal.Text = objectiveGoal ?? string.Empty; // Use empty string if null
                txtDimensionStr.Text = dimensionStr ?? string.Empty; // Use empty string if null
            }
            else
            {
                // Clear text values for subsequent rows
                txtObjectiveGoal.Text = string.Empty;
                txtDimensionStr.Text = string.Empty;
            }
        }
    }
        ...
        try
        {
            ...
            if (gv_AppraisalFunc.Rows.Count > 0)
            {
                for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
                {
                    ...
                    TextBox txtKpiWeight = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtKpiWeight");




                    if (txtKpiWeight.Text == "")
                    {
                        weightTotal = weightTotal + 0;
                    }
                    else
                    {
                        weightTotal = weightTotal + Convert.ToDecimal(txtKpiWeight.Text.ToString());
                    }
                    if (txtMark.Text == "")
                    {
                        markTotal = markTotal + 0;
                ...
                Label lblTotalKpiWeight = (Label)gv_AppraisalFunc.FooterRow.FindControl("lblTotalKpiWeight");
                Label lblTotalMarkAchiv = (Label)gv_AppraisalFunc.FooterRow.FindControl("lblTotalMarkAchiv");
                Label lblMarkAchiv = (Label)gv_AppraisalFunc.FooterRow.FindControl("lblMarkAchiv");
                lblTotalKpiWeight.Text = weightTotal.ToString();

                decimal _tWeight = 0;
                try
                {
                    _tWeight = Convert.ToDecimal(lblTotalKpiWeight.Text);
                }
                catch
                {
                ...
                try
                {
                    _tself = Convert.ToDecimal(tst2.Text);
                }
                catch
                {
                ...
                lblTotalMarkAchiv.Text = "Total Achi: " + res.ToString()+"%";
                lblMarkAchiv.Text = "Total Out of 75: " + resMain.ToString();
            }
        }
        catch
        {
    ...
    protected void PartBOwn_OnClick(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            DataTable dt = _appPartA.GetAppraisalSelf(Convert.ToInt32(selfMaster));
            id_mastetID.Value = masterID.ToString();
            DataTable dt33 = _appPartA.GetAppraisalPartB_new(masterID);
            DataTable dt3 = _appPartA.GetAppraisalSelfB(Convert.ToInt32(selfMaster));
            ViewState["PARTB"] = dt3;
    ...
    protected void PartC_OnClick(object sender, EventArgs e)
    {
        ...
        if (FinancialYearDropDownList.SelectedValue != "")
        {
            ...
            DataTable dt = _appraisalPartBdal.GetAppraiSalOKRBSCInfo(Convert.ToInt32(FinancialYearDropDownList.SelectedValue), Convert.ToInt32(empID.Value));
            if (dt.Rows.Count > 0)
            {
                M = dt.Rows[0]["OptionInfo"].ToString();
            }

            System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('MBSCOKRDetailsViewSelf.aspx?EmpInfoId=" + empID.Value + "&financialYearId=" + FinancialYearDropDownList.SelectedValue + "&M="+ M + "' ,'_blank');", true);
    ...
    protected void radOp_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        if (FinancialYearDropDownList.SelectedValue != "")
        {
            ...
            DataTable dt = _appDashboard.GetAppraisalDashboardOwn333fin(Convert.ToInt32(Session["EmpInfoId"]),
                             Convert.ToInt32(FinancialYearDropDownList.SelectedValue), "  AND   (tblMBSCAppraisalMasterAppLog.Version=CELog.MaxVer  or  tblMBSCAppraisalMasterAppLog.Version is null )   ", FinancialYearDropDownList.SelectedItem.Text);
            if (dt.Rows.Count > 0)
            {
            ...
            if (PartAOwn.Text != "Not Complete" && PartBOwn.Text != "Not Complete")
            {
                ...
                if (IsMidYearStatus.Checked == false)
                {
                    ...
                    if (ddd == "True")
                    {
                        ...
                        SenMailForApprved(appLogDao1.ForEmpInfoId, "BSC Mid-Year  Approval ", @"    Dear Sir,  
A Employee's BSC Mid-Year is waiting for your approval.  
 please login with the below link.   http://182.160.103.234:8088/
");
                    }
...
");
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(),
                                 "alert",
                                 "alert('Operation Successful...');window.location ='MOKRBSCAppraisalDashboardSelf.aspx';",
                                 true);
            }
            else
            {
    ...
    protected void btn_print_OnClick(object sender, EventArgs e)
    {
        ...
        DataTable dt = _appraisalPartBdal.GetAppraiSalOKRBSCInfo(Convert.ToInt32(FinancialYearDropDownList.SelectedValue), Convert.ToInt32(id_empId.Value));
        if (dt.Rows.Count > 0)
        {
            M = dt.Rows[0]["OptionInfo"].ToString();
        }
        url = "../Report_UI/MOKRBSCInformationReportViewer.aspx?EmpInfoId=" + id_empId.Value + "&financialYearId=" + FinancialYearDropDownList.SelectedValue + "&M=" + M;
    ...
    protected void btnFunctionalSave_OnClick(object sender, EventArgs e)
    {
        if (ValidationlFunctional() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
            {
                ...
                HiddenField hdnDimensionStr = (HiddenField)gv_AppraisalFunc.Rows[i].FindControl("hdnDimensionStr");
                HiddenField hdnObjectiveGoal = (HiddenField)gv_AppraisalFunc.Rows[i].FindControl("hdnObjectiveGoal");
                Label txtKPIMeasure = (Label)gv_AppraisalFunc.Rows[i].FindControl("txtKPIMeasure");
                Label txtInitiatives = (Label)gv_AppraisalFunc.Rows[i].FindControl("txtInitiatives"); 
                DropDownList txtMidStatus = (DropDownList)gv_AppraisalFunc.Rows[i].FindControl("txtMidStatus"); 
                ...
                TextBox txtselfMark = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtselfMark");
                TextBox txtSuperMark = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtMark");
                TextBox txtKpiWeight = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtKpiWeight");
                TextBox txtDeadLine = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtDeadLine");
                TextBox txtComment = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtComment");
                ...
                if (hdnDimensionStr.Value != "" )
                {
                    ...
                    { 
                        ...
                        area.Dimension = Convert.ToInt32(gv_AppraisalFunc.DataKeys[i][2].ToString());
                    }
                    catch (Exception)
                    {
                    ...
                    area.SelfMark = string.IsNullOrEmpty(txtselfMark.Text) ? (decimal?)null : Convert.ToDecimal(txtselfMark.Text.Trim());
                  ...
                  //area.SupervisorMark = null;
                    ...
                    area.CommentsFunc = txtComment == null ? "" : txtComment.Text.Trim();
    ...
    private bool ValidationlFunctional()
    {
        ...
        for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
        {
            ...
            if (txtMidStatus.SelectedValue == "0")
            {
                isVAlid = false;
                aShowMessage.ShowMessageBox("Please select Mid-Year Status", this);
                txtMidStatus.Focus();
                break;
            }
            if (txtMidStatus.SelectedValue != "Not Started"
                && txtMidStatus.SelectedValue != "Ongoing"
                && txtMidStatus.SelectedValue != "Not Relevant")
            {
                ...
                {
                    isVAlid = false;
                    aShowMessage.ShowMessageBox("Please Enter Mid-Year Marking", this);
                    txtselfMark.Focus();
                    break;
                }
            }
        ...
        //{
        //    isVAlid = false;
        //    aShowMessage.ShowMessageBox("Total Weight Can Not be 0!", this);
    ...
    protected void txtselfMark_OnTextChanged(object sender, EventArgs e)
    {
        ...
        int rowIndex = ((GridViewRow)(((TextBox)sender).Parent.Parent)).RowIndex;
        Label lblAutoCalculation = (Label)gv_AppraisalFunc.Rows[rowIndex].FindControl("lblAutoCalculation");   
        Label lblAutoCalculationPer = (Label)gv_AppraisalFunc.Rows[rowIndex].FindControl("lblAutoCalculationPer");     
         
        TextBox txtselfMark = (TextBox)gv_AppraisalFunc.Rows[rowIndex].FindControl("txtselfMark");
        TextBox txtKpiWeight = (TextBox)gv_AppraisalFunc.Rows[rowIndex].FindControl("txtKpiWeight");
        ...
        try
        {
            selfMark = Convert.ToDecimal(txtselfMark.Text);
        }
        catch
        {
        ...
        try
        {
            KpiWeight = Convert.ToDecimal(txtKpiWeight.Text);
        }
        catch
        {
        ...
        if (selfMark > AutoCalculation)
        {
            ...
            txtselfMark.Text = AutoCalculation.ToString();
            aShowMessage.ShowMessageBox("Cannot be greater than Maximum Mid-Year Marking.", this);
    ...
    private MOKRBSCAppraisalPartBDAL _appraisalPartBdal = new MOKRBSCAppraisalPartBDAL();
        ...
        if (ValidationBehave() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalPartB.Rows.Count; i++)
            {
                ...
                if (txtSkillInfo.Text.Trim().ToString() != "" && txtSelfScore.Text.Trim().ToString() != "")
                {
                    ...
                    area.BSCAppraisalSelfBehaveId = Convert.ToInt32(gv_AppraisalPartB.DataKeys[i][0].ToString());
    ...
    protected void txtMidStatus_SelectedIndexChanged(object sender, EventArgs e)
    {
        int rowIndex = ((GridViewRow)(((DropDownList)sender).Parent.Parent)).RowIndex;
        DropDownList ddlMidStatus = (DropDownList)gv_AppraisalFunc.Rows[rowIndex].FindControl("txtMidStatus");
        bool disableMark = IsMidYearMarkDisabled(ddlMidStatus);
        SetMidYearInputsState(gv_AppraisalFunc.Rows[rowIndex], disableMark, disableMark);
        ApplyMidYearStatusColor(ddlMidStatus);
        CalculateB();
    }
}
