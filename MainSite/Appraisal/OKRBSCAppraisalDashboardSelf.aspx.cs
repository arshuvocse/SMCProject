...
public partial class Appraisal_OKRBSCAppraisalDashboardSelf : System.Web.UI.Page
{
                ...
                if (radOp.SelectedValue == "Own")
                {
                    if (FinancialYearDropDownList.SelectedValue != "")
                    {
                        ...
                        DataTable dt = _appDashboard.GetAppraisalDashboardOwn333fin(Convert.ToInt32(Session["EmpInfoId"]),
                            Convert.ToInt32(FinancialYearDropDownList.SelectedValue), "  AND tblBSCAppraisalMasterAppLog.Version=CELog.MaxVer  ", FinancialYearDropDownList.SelectedItem.Text);
                        if (dt.Rows.Count > 0)
                        {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < AppraisalOwn.Rows.Count; i++)
                            {
                                ...
                                DataTable dt3 = id_appraisalMaster.Value == "0"
                                    ? new DataTable()
                                    : _appDashboard.GetAppraisalByPermission3(id_appraisalMaster.Value);
                                string EmpID = "";
                                string Actions = "";
                                if (dt3.Rows.Count>0)
                                {
                                ...
                                DataTable dt345 = id_appraisalMaster.Value == "0"
                                    ? new DataTable()
                                    : _appDashboard.GetAppraisalByPermission3(id_appraisalMaster.Value);
                                string EmpIDa = "";
                                string Actionsa = "";
    ...
    protected void PartAOwn_OnClick(object sender, EventArgs e)
    {
        ...
        catch (Exception)
        {
            ...
            for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
            {
                ...
                Label lblAutoCalculationPer = (Label)gv_AppraisalFunc.Rows[i].FindControl("lblAutoCalculationPer");
            ...
            if (txtMidStatus.Text== "Not Started")
            {
             //   txtselfMark.Text = "";
                txtResultYearEnd.Text = "";
             //   txtselfMark.ReadOnly = true;
              //  lblAutoCalculationPer.Text = "Achi: 0%";
            }
    ...
    {
        //// Ensure that we only modify data rows, not header or footer rows
        //if (e.Row.RowType == DataControlRowType.DataRow)
        //{
        //    // If the row is not the first row (index > 0), we clear the text
        //    if (e.Row.DataItemIndex > 0)
        //    {
        //        // Find the controls by ID and clear their text for non-first rows
        //        var txtDimensionStr = (Label)e.Row.FindControl("txtDimensionStr");
        //        var txtObjectiveGoal = (Label)e.Row.FindControl("txtObjectiveGoal");

        //        if (txtDimensionStr != null)
        //            txtDimensionStr.Text = string.Empty;

        //        if (txtObjectiveGoal != null)
    ...
    protected void PartC_OnClick(object sender, EventArgs e)
    {
        ...
        if (FinancialYearDropDownList.SelectedValue != "")
        {
            ...
            System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('BSCOKRDetailsViewSelf.aspx?EmpInfoId=" + empID.Value + "&financialYearId=" + FinancialYearDropDownList.SelectedValue + "&M="+ M + "' ,'_blank');", true);
    ...
    protected void btn_Save_OnClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                int appraisalMasterId;
                if (!Int32.TryParse(mastrId.Value, out appraisalMasterId) || appraisalMasterId <= 0)
                {
                    DataTable existingMaster = _appPartA.GetAppraisalMasterIdFromAppraisalSelfMasterId(Convert.ToInt32(selfMaster.Value));

                    if (existingMaster.Rows.Count == 0)
                    {
                        _appPartA.SaveAppraisalMasterFromAppraisalSelf(selfMaster.Value);
                        existingMaster = _appPartA.GetAppraisalMasterIdFromAppraisalSelfMasterId(Convert.ToInt32(selfMaster.Value));
                    }

                    if (existingMaster.Rows.Count == 0)
                    {
                        AlertMessageBoxShow("Appraisal master information was not created. Please contact with HR Department !!!");
                        return;
                    }

                    mastrId.Value = existingMaster.Rows[0]["BSCAppraisalMasterId"].ToString();
                }
...
");
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(),
                                 "alert",
                                 "alert('Operation Successful...');window.location ='OKRBSCAppraisalDashboardSelf.aspx';",
                                 true);
            }
            else
            {
    ...
    protected void btn_print_OnClick(object sender, EventArgs e)
    {
        ...
        DataTable dt = _appraisalPartBdal.GetAppraiSalOKRBSCInfo(Convert.ToInt32(id_finYear.Value), Convert.ToInt32(id_empId.Value));
        if (dt.Rows.Count > 0)
        {
            M = dt.Rows[0]["OptionInfo"].ToString();
        }
        url = "../Report_UI/BSCKPIInformationReportViewer.aspx?EmpInfoId=" + id_empId.Value + "&financialYearId=" + id_finYear.Value + "&M=" + M;
    ...
    protected void btnFunctionalSave_OnClick(object sender, EventArgs e)
    {
        if (ValidationlFunctional() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalFunc.Rows.Count; i++)
            {
                ...
                TextBox txtSelfAssessment = (TextBox)gv_AppraisalFunc.Rows[i].FindControl("txtSelfAssessment");
                ...
                if (hdnDimensionStr.Value != "" )
                {
                    ...
                    area.SelfAssessment = txtSelfAssessment.Text.Trim().ToString();
                   ...
                   // area.SupervisorMark = string.IsNullOrEmpty(txtSuperMark.Text) ? 0 : Convert.ToDecimal(txtSuperMark.Text.Trim().ToString());
    ...
    protected void txtselfMark_OnTextChanged(object sender, EventArgs e)
    {
        ...
        if (selfMark > AutoCalculation)
        {
            ...
            aShowMessage.ShowMessageBox("Self mark cannot be greater than Maximum Self-Mark.", this);
    ...
    protected void btnBehave_OnClick(object sender, EventArgs e)
    {
        ...
        if (ValidationBehave() == true)
        {
            ...
            for (int i = 0; i < gv_AppraisalPartB.Rows.Count; i++)
            {
                ...
                TextBox txtSelfScore = (TextBox)gv_AppraisalPartB.Rows[i].FindControl("SelfScore");
                Label MidSetScore = (Label)gv_AppraisalPartB.Rows[i].FindControl("MidSetScore");
                ...
                if (txtSkillInfo.Text.Trim().ToString() != "" && txtSelfScore.Text.Trim().ToString() != "")
                {
                    ...
                    try
                    {
                        area.MidYearSuperScore = Convert.ToDecimal(MidSetScore.Text.ToString());
                    }
                    catch
                    {
                        area.MidYearSuperScore = 0;
                    }
                    area.Comments = "";
    ...
    protected void lbMidStatus_OnClick(object sender, EventArgs e)
    {
        ...
        int rowID = gvRow.RowIndex;
            ...
            TextBox txtMidStatus = (TextBox)gv_AppraisalFunc.Rows[rowID].FindControl("txtMidStatus");

                bool status2 = _appDashboard.UpdateMidYearStatus(Convert.ToInt32(area.AppraisalSelfFucAreaId), txtMidStatus.Text.ToString());
                if (status2)
                {
    ...
    protected void ptOverallSta_Click(object sender, EventArgs e)
    {
        LinkButton lb = (LinkButton)sender;
        GridViewRow gvRow = (GridViewRow)lb.NamingContainer;
        int rowID = gvRow.RowIndex;
        ...
        if (FinancialYearDropDownList.SelectedValue != "")
        {
            ...
            System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "openModal", "window.open('BSCOKRDetailsView.aspx?EmpInfoId=" + empID.Value + "&financialYearId=" + FinancialYearDropDownList.SelectedValue + "' ,'_blank');", true);
            //Response.Redirect("KPIInformationDetailsView.aspx?EmpInfoId=" + empID.Value + "&financialYearId=" + FinancialYearDropDownList.SelectedValue + "");
        }
        else
        {
