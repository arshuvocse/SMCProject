...
public partial class Increment_UI_IncrementEntryEdit : System.Web.UI.Page
{
    ...
    private void GetOneData(string id)
    {
        ...
        if (dtMain.Rows.Count > 0)
        {
            ...
            ddlIncrementType.SelectedValue=
                dtMain.Rows[0]["IncrementTypeId"].ToString();
                ...
                for (int i = 0; i < loadGridView.Rows.Count; i++)
                {
                  ...
                  //  var ddlDesignation = (DropDownList)loadGridView.Rows[i].Cells[0].FindControl("designationDropDownList");
                   ...
                   // var ddlDepartment = (DropDownList)loadGridView.Rows[i].Cells[0].FindControl("departmentDropDownList");
                    ...
                    ddlIncrementalStep.SelectedValue = dtMain.Rows[0].Field<Int32>("IncrementalStepId").ToString(CultureInfo.InvariantCulture);


                    if (dtMain.Rows[0]["AutoProcess"].ToString() == "Manually Updated")
                    {
                        manualUpdateCheckBox.Items[1].Selected = true;
                    }
                    else
                    {
                        manualUpdateCheckBox.Items[0].Selected = true;
