...
public partial class UserSetup_EmployeeInfoLisImg : System.Web.UI.Page
{
    ...
    protected void ButtonUpdateAll_Click(object sender, EventArgs e)
    {
        foreach (GridViewRow row in loadGridView.Rows)
        {
            ...
            {
                string employeeId = loadGridView.DataKeys[row.RowIndex].Value.ToString();
                TextBox textBoxNewImageName = (TextBox)row.FindControl("TextBoxNewImageName");
                HiddenField EmpImage = (HiddenField)row.FindControl("EmpImage");
                HiddenField sEmpMasterCode = (HiddenField)row.FindControl("sEmpMasterCode");
                string newImageName = textBoxNewImageName.Text.Trim();

                // Validate the new image name
                if (string.IsNullOrEmpty(newImageName))
                {
                    // Show error message (not shown in this example)
                    continue;
                }
                ...
                // Create the current and new file paths
                string currentFilePath = Path.Combine(folderPath, EmpImage.Value);
                string newFilePath = Path.Combine(folderPath, sEmpMasterCode.Value + ".jpg");
                ...
                try
                {
                    ...
                    // Update your data source with the new image name (not shown in this example)
                }
                catch (Exception ex)
                {
                    // Handle exceptions (not shown in this example)
                }
            }
        }
