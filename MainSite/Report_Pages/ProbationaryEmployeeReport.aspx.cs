...
public partial class Report_Pages_ProbationaryEmployeeReport : System.Web.UI.Page
{
    ShowMessage aShowMessage = new ShowMessage();
    ProbationaryEmployeeReportDal aReportDal = new ProbationaryEmployeeReportDal();
    ...
    {
        Response.Redirect("ProbationaryEmployeeReport.aspx");
    }
    ...
    protected void btn_Approve_OnClick(object sender, EventArgs e)
    {
        ...
        if (ApproveValidation())
        {
            for (int i = 0; i < gv_ProbationaryEmployee.Rows.Count; i++)
            {
                ...
                {
                    var aListDao = new ProbationListDao();
                    ...
                    aListDao.JoiningDate = Convert.ToDateTime(gv_ProbationaryEmployee.Rows[i].Cells[3].Text.Trim());
                    aListDao.ProbationEndDate = Convert.ToDateTime(gv_ProbationaryEmployee.Rows[i].Cells[4].Text.Trim());
                    aListDao.Approveby = Session["LoginName"].ToString();
                    aListDao.ApproveDate = DateTime.Now;

                    aReportDal.SaveProbationConfirmationList(aListDao);
                }
            }
