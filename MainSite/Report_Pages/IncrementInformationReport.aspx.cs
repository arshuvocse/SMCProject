...
public partial class Report_Pages_IncrementInformationReport : System.Web.UI.Page
{
    ShowMessage aShowMessage = new ShowMessage();
    IncrementInformationReportDal aReportDal = new IncrementInformationReportDal();
    ...
    {
        DataTable dataTable = aReportDal.LoadInfoIncrementInfoDAL(IncrementGenerateParamiterList(), IncrementGenerateParamiterListSP());
    ...
    private String IncrementGenerateParamiterList()
    {
        ...
        if (ddlCompany.SelectedValue != "")
        {
            parameter = parameter + " AND tblIncrement.CompanyId = " + ddlCompany.SelectedValue;
        }
        ...
        {
            parameter = parameter + " AND tblIncrement.EffectiveDate BETWEEN '" + IncrementFromDt.Text + "' AND '" + IncrementToDt.Text + "' ";
        }
        if (IncrementFromDt.Text != string.Empty && IncrementToDt.Text == string.Empty)
        {
            parameter = parameter + " AND tblIncrement.EffectiveDate BETWEEN '" + IncrementFromDt.Text + "' AND '" + DateTime.Now.ToString("dd-MMM-yyyy") + "' ";
        }

        if (IncrementFromDt.Text == string.Empty && IncrementToDt.Text != string.Empty)
        {
            parameter = parameter + " AND tblIncrement.EffectiveDate BETWEEN '" + IncrementToDt.Text + "' AND '" + IncrementToDt.Text + "' ";
        }
        ...
        if (ddlIncrementType.SelectedValue != "")
        {
            parameter = parameter + " AND EPE.IncrementTypeId = " + ddlIncrementType.SelectedValue;
        }

        if (chkContratualIncrement.Checked)
        {
            parameter = parameter + " AND EPE.IncrementTypeId = 2 AND EPE.EmpTypeId = 2 ";
        }

        return parameter;
    }

    private String IncrementGenerateParamiterListSP()
    {
        string parameter = "   ";
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
        ...
        {
            string attachment = "attachment; filename=Increment_List_" + DateTime.Now.ToString("dd-MMM-yyyy") +
                                ".xls";
   ...
   <h3>Annual/Monthly Increment List
</h3>
    ...
    protected void btn_Approve_OnClick(object sender, EventArgs e)
    {
        if (ApproveValidation())
        {
            for (int i = 0; i < gv_ProbationaryEmployee.Rows.Count; i++)
            {
                ...
                {
                    var aListDao = new IncrementConfirmationDao();
                    ...
                    aListDao.EffectiveDate = Convert.ToDateTime(gv_ProbationaryEmployee.Rows[i].Cells[3].Text.Trim());
                    aListDao.PF = Convert.ToDecimal(gv_ProbationaryEmployee.Rows[i].Cells[4].Text.Trim());
