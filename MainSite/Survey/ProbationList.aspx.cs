        ...
        if (ddlCompany.SelectedValue != "")
        {
            ...
            if (dtuserType.Rows.Count > 0)
            {
                if (dtuserType.Rows[rowIndex].Field<Int32>("UserTypeId").ToString(CultureInfo.InvariantCulture) == "4")
                {
                    try
                    {
                    DataTable dtdata = aDal.LoadEmployeeDataBySuperAdmin(Parameter());
                ...
                else
                {
                    try
                    {
                        DataTable dtdata2 = aDal.LoadEmployeeData(Parameter());
    ...
    protected void loadGridView_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Evaluate")
        {
            ...
            if (datKey != null)
            {
                ...
                DataTable dt = aDal.LoadEmployeeCheckforProbition( (id));

                if (dt.Rows.Count>0)
                {
                    showMessageBox("ALready submitted for Approval!!!");
                    //string did = datKey["MPBudgetDetailsId"].ToString();
                    //Session["Status"] = "Edit";
