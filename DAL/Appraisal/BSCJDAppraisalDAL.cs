...
{
    public class BSCJDAppraisalDAL
    {
 ...
 desg.Designation, Dpt.DepartmentName  FROM   tblBSCKPIDeadLineDetails dtls
 ...
 WHERE        reff.ShowCompany in (ComAssain)  and a.IsActive=1 "+param+") tbl";
        ...
        public int SaveKpiSetupMaster(KpiDeadlineMaster aMaster, string user)
        {
            try
            {
                ...
                if (aMaster.KPIDeadLineMasterId == 0)
                {
                    ...
                    string query =
                        @"insert into tblBSCKpiDeadlineMaster (CompanyId, FinancialYearId, IsCommon ,EntryDate, EntryBy,Subject, DeclarationDate,FYDes_BSCDec) values(@CompanyId, @FinancialYearId, @IsCommon ,@EntryDate, @EntryBy,@Subject, @DeclarationDate,  (SELECT FinancialYearDesc 
                ...
                else
                {
                    ...
                    string query = @"update tblBSCKpiDeadlineMaster set CompanyId = @CompanyId,FYDes_BSCDec=(SELECT FinancialYearDesc 
                    FROM dbo.tblFinancialYear 
                    WHERE FinancialYearId =@FinancialYearId ) ,Subject=@Subject, FinancialYearId = @FinancialYearId , IsCommon = @IsCommon , UpdateBy = @UpdateBy , UpdateDate = @UpdateDate, DeclarationDate=@DeclarationDate where KPIDeadLineMasterId = @KPIDeadLineMasterId ";
        ...
        public bool SaveKpiSetupDetails(List<KPIDeadLineDetails> aDetails, int master)
        {
            try
            {
                ...
                bool result = false;
                string delQ = @"delete from tblBSCAppraisalDeadLineDetails where BSCAppraisalDeadLineMasterId = " + master + "";
                ...
                foreach (KPIDeadLineDetails item in aDetails)
                {
                    ...
                    aParameters.Add(new SqlParameter("@OptionInfo", item.OptionInfo));
                    aParameters.Add(new SqlParameter("@KPIDeadLineMasterId", master));

                    string query = @"insert into tblBSCAppraisalDeadLineDetails (BSCAppraisalDeadLineMasterId, EmpinfoId, DeadLine, Remarks,OptionInfo) 
                                                                values(@KPIDeadLineMasterId, @EmpinfoId, @DeadLine, @Remarks,@OptionInfo)";
         ...
         public bool SaveBSCOKRPerDetails(List<KPIDeadLineDetails> aDetails,int FinancialYearId, int master)
        {
            try
            {

                bool result = false;
                string delQ = @"delete from tblBSCOKRPermission where KPIDeadLineMasterId = " + master + "";
                ...
                foreach (KPIDeadLineDetails item in aDetails)
                {
                    ...
                    aParameters.Add(new SqlParameter("@OperationType", item.OptionInfo));
                    aParameters.Add(new SqlParameter("@KPIDeadLineMasterId", master));

                    string query = @"INSERT INTO [dbo].[tblBSCOKRPermission]
           ([KPIDeadLineMasterId]
           ,[FinancialYearId]
           ,[EmpInfoId]
           ,[OperationType])
     VALUES
            ...
            //string param
        {
            ...
            try
            {
                string query = @"select a.KPIDeadLineMasterId , c.CompanyName ,d.FinancialYearDesc , b.TotalEmployee  , CONVERT(nvarchar (11),a.EntryDate , 106)EntryDate ,a.EntryBy,*   from tblBSCKpiDeadlineMaster A 
                                    left join tblCompanyInfo c on a.CompanyId = c.CompanyId
                                    left join tblFinancialYear d on a.FinancialYearId = d.FinancialYearId
                                    
                                    left join (select count(EmpinfoId)TotalEmployee , KPIDeadLineMasterId from tblBSCKPIDeadLineDetails group by KPIDeadLineMasterId) B on a.KPIDeadLineMasterId = b.KPIDeadLineMasterId  
        ...
        public DataTable GetKpiSetupByMaster(int id)
        {
            try
            {
                string query = @"select  tblBSCOKRPermissionMaster.EntryDate  UpdateDate,  * from tblBSCOKRPermissionMaster where KPIDeadLineMasterId = " + id + "";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetKpiSetupByMasterDeadLine(int id)
        {
            try
            {
                string query = @"select  tblBSCAppraisalDeadlineMaster.EntryDate  UpdateDate,  * from tblBSCAppraisalDeadlineMaster where BSCAppraisalDeadLineMasterId = " + id + "";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        {
            string query = @"select * from  tblBSCKpiDeadlineMaster where CompanyId =" +  CompanyId  + " and FinancialYearId =  " +  FinancialYearId ;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }

        
         public DataTable BSCDeleteValidattionForEffectiveDate(string CompanyId, string FinancialYearId, string OptionInfo)
        {
            string query = @"

 select distinct  OptionInfo  from  tblBSCAppraisalDeadlineMaster  a 
     left join (select count(EmpinfoId)TotalEmployee , BSCAppraisalDeadLineMasterId KPIDeadLineMasterId, OptionInfo from tblBSCAppraisalDeadLineDetails group by BSCAppraisalDeadLineMasterId,OptionInfo) B on a.BSCAppraisalDeadLineMasterId = b.KPIDeadLineMasterId  

	 where OptionInfo='" + OptionInfo+@"' and CompanyId =" +  CompanyId  + " and FinancialYearId =  " +  FinancialYearId ;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }

        
         public DataTable BSCPermissionDeleteValidattionForEffectiveDate(string CompanyId, string FinancialYearId, string OptionInfo)
        {
            string query = @"
		select distinct  OptionInfo  from  tblBSCOKRPermissionMaster  a 
     left join (select count(EmpinfoId)TotalEmployee , KPIDeadLineMasterId,OperationType OptionInfo from tblBSCOKRPermission group by KPIDeadLineMasterId,OperationType) B on a.KPIDeadLineMasterId = b.KPIDeadLineMasterId  
	 ...
	 where OptionInfo='" + OptionInfo+@"' and CompanyId =" +  CompanyId  + " and FinancialYearId =  " +  FinancialYearId ;
        ...
        {
            string query = @"select * from  tblBSCAppraisalSelfMaster where EmpInfoId =" + empInfoId + " and FinancialYearId =  " + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }
        
      
        public DataTable CheckPermisBSCKepiSetpExist(string empInfoId, string FinancialYearId)
        {
            string query = @"select * from  tblBSCOKRPermission where EmpInfoId =" + empInfoId + " and FinancialYearId =  " + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }
        
      
            public bool DeleteDetailDeclare(string empInfoId, int masterID)
            {
            bool result = false;
            try
                {
                    ...
                    string delQ = @"delete from tblBSCKPIDeadLineDetails where EmpinfoId = " + empInfoId + "  and  KPIDeadLineMasterId = " + masterID;
            ...
            public bool DeletePermisDetailDeclare(string empInfoId, int masterID)
            {
            bool result = false;
            try
                {
                    ...
                    string delQ = @"delete from tblBSCOKRPermission where EmpinfoId = " + empInfoId + "  and  KPIDeadLineMasterId = " + masterID;
        ...
        {
            string query = @"select * from  tblBSCAppraisalSelfMaster where AppraisalSelfMasterId is not null and EmpInfoId =" + empInfoId + " and FinancialYearId =  " + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }
        ...
        {
            string query = @"select * from  tblBSCAppraisalSelfMaster where AppraisalSelfMasterId is not null and   EmpInfoId  in (" + empInfoId + ") and FinancialYearId =  " + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }

        public DataTable CheckKepiSetpExistMulti(string empInfoId, string FinancialYearId)
        {
            string query = @"select * from  tblBSCAppraisalSelfMaster where  EmpInfoId  in (" + empInfoId + ") and FinancialYearId =  " + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }
...
left JOIN dbo.tblDesignation desg ON  empInfo.DesignationId=desg.DesignationId
left JOIN dbo.tblDepartment Dpt ON  empInfo.DepartmentId=Dpt.DepartmentId
left JOIN dbo.tblBSCKpiDeadlineMaster mas ON  mas.KPIDeadLineMasterId=dtls.KPIDeadLineMasterId
 ...
 where dtls.KPIDeadLineMasterId = " +
        ...
        public DataTable GetBSCKpiSetupDetailsByMaster(int id)
        {
            try
            {
                string query = @"SELECT dtls.OptionInfo OptionInfo, mas.FinancialYearId,   dtls.EmpinfoId,BSCAppraisalDeadLineDetailsId KPIDeadLineDetailsId, dtls.BSCAppraisalDeadLineMasterId KPIDeadLineMasterId, empInfo.EmpMasterCode, empInfo.EmpName,   dtls.DeadLine DeadLine, dtls.Remarks Remarks, 0 AS DivisionName,
 desg.Designation, Dpt.DepartmentName,*  FROM   tblBSCAppraisalDeadLineDetails dtls
INNER JOIN dbo.tblEmpGeneralInfo empInfo ON  dtls.EmpinfoId=empInfo.EmpInfoId
left JOIN dbo.tblDesignation desg ON  empInfo.DesignationId=desg.DesignationId
left JOIN dbo.tblDepartment Dpt ON  empInfo.DepartmentId=Dpt.DepartmentId
left JOIN dbo.tblBSCAppraisalDeadlineMaster mas ON  mas.BSCAppraisalDeadLineMasterId=dtls.BSCAppraisalDeadLineMasterId
 where dtls.BSCAppraisalDeadLineMasterId = " +
                    id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception exception)
        ...
        public bool DeleteKpiSetup(int master, string user)
        {
            try
            {
                ...
                string query = @"update tblBSCKpiDeadlineMaster set IsDelete = 1  , DeleteBy = @DeleteBy ,DeleteDate = @DeleteDate where KPIDeadLineMasterId = @KPIDeadLineMasterId ";
...
SELECT [KPIDeadLineMasterId]
      ...
      ,[Subject]
      ,[DeclarationDate]
  FROM [dbo].[tblBSCKpiDeadlineMaster] where KPIDeadLineMasterId=@KPIDeadLineMasterId
...
SELECT  [KPIDeadLineDetailsId]
      ...
      ,[ExtensionDate]
      ,[IsMailSend]
  FROM [dbo].[tblBSCKPIDeadLineDetails] where KPIDeadLineMasterId=@KPIDeadLineMasterId


DELETE FROM [dbo].[tblBSCKpiDeadlineMaster]
      WHERE  KPIDeadLineMasterId=@KPIDeadLineMasterId



DELETE FROM [dbo].[tblBSCKPIDeadLineDetails]
      WHERE  KPIDeadLineMasterId=@KPIDeadLineMasterId
...
";
        ...
        public DataTable CheckPreviousKpiDeadline(int com, int fin, int masterId)
        {
            try
            {
                string query = @"select * from tblBSCKpiDeadlineMaster where CompanyId =" + com + " and FinancialYearId = " + fin + " and KPIDeadLineMasterId!= " + masterId + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetBSCEmployeeForKpiSetUpNew_ST(string companyId, string deadLine, string remarks, string OptionInfo, string param)
        {
            try
            {
                string query = @" select distinct  * from (select  A.EmpInfoId, A.EmpMasterCode , A.EmpName , desg.Designation ,dpt.DepartmentName , div.DivisionName , '" + deadLine + @"' as DeadLine , '" + remarks + @"' as Remarks , '" + OptionInfo + @"' as OptionInfo  from tblEmpGeneralInfo A  
left join tblDivision div on a.DivisionId = div.DivisionId  
left join tblDepartment dpt on a.DepartmentId = dpt.DepartmentId 
left join tblDesignation desg on a.DesignationId = desg.DesignationId   where A.CompanyId = " + companyId + " and a.IsActive=1 " + param + "" +
                               @"    union all  
select  A.EmpInfoId, A.EmpMasterCode , A.EmpName , desg.Designation ,dpt.DepartmentName , div.DivisionName , '" + deadLine + @"' as DeadLine , '" + remarks + @"' as Remarks , '" + OptionInfo + @"' as OptionInfo  from tblEmpGeneralInfo A  
left join tblDivision div on a.DivisionId = div.DivisionId  
left join tblDepartment dpt on a.DepartmentId = dpt.DepartmentId 
left join tblDesignation desg on a.DesignationId = desg.DesignationId  
inner JOIN   tblEmpAllRefference reff  ON A.EmpinfoId = reff.RefferenceEmpId 
 ...
 WHERE        reff.ShowCompany in (ComAssain)  and a.IsActive=1 " + param + ") tbl";
   ...
   public DataTable GetBSCOKRPermission(string companyId, string deadLine, string remarks, string OptionInfo, string param)
        {
            try
            {
                string query = @" select distinct  * from (select  A.EmpInfoId, A.EmpMasterCode , A.EmpName , desg.Designation ,dpt.DepartmentName , div.DivisionName , '" + deadLine + @"' as DeadLine , '" + remarks + @"' as Remarks , '" + OptionInfo + @"' as OptionInfo  from tblEmpGeneralInfo A  
left join tblDivision div on a.DivisionId = div.DivisionId  
inner join tblBSCOKRPermission prm on A.EmpInfoId = prm.EmpInfoId     
left join tblDepartment dpt on a.DepartmentId = dpt.DepartmentId 
left join tblDesignation desg on a.DesignationId = desg.DesignationId   where A.CompanyId = " + companyId + " and a.IsActive=1 " + param + "" +
                               @"    union all  
select  A.EmpInfoId, A.EmpMasterCode , A.EmpName , desg.Designation ,dpt.DepartmentName , div.DivisionName , '" + deadLine + @"' as DeadLine , '" + remarks + @"' as Remarks , '" + OptionInfo + @"' as OptionInfo  from tblEmpGeneralInfo A  
...
";
        ...
        public DataTable GetDetailsDataByMasterid(int mid)
        {
            try
            {
                string query = @"SELECT EmpinfoId,DeadLine,Remarks FROM dbo.tblBSCKPIDeadLineDetails WHERE KPIDeadLineMasterId = " + mid;
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
