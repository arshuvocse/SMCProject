...
{
    public class BSCAppraisalFunctionalPartDAL
    {
        ClsCommonInternalDAL _aCommonInternalDal = new ClsCommonInternalDAL();
        DataAccessManager _aAcessManager = new DataAccessManager();

        public bool UpdateAgrreeDisAgree(bool IsAgreed, bool IsDisagreed, string AppraisalMasterId)
        {
            ...
            try
            {
                ...
                //if (id.JdMasterId > 0)
                {
                    ...
                    aParameters.Add(new SqlParameter("@IsAgreed", IsAgreed));
                    aParameters.Add(new SqlParameter("@IsDisagreed", IsDisagreed));


                    string query =
                        @"UPDATE [dbo].[tblBSCAppraisalMaster]
   SET    [IsAgreed] =@IsAgreed
      ,[IsDisagreed] = @IsDisagreed
      ,[AggreeDisDate] = GETDATE()
      
 WHERE BSCAppraisalMasterId=@AppraisalMasterId";
        ...
        public DataTable GetEmployeeDetailsKPIREport(int id, int FinancialYearId)
        {
            try
            {
                string query = @"declare @IsAgreed bit=0
declare @IsDisagreed bit=0
  SELECT @IsAgreed= ISNULL(IsAgreed,0)  , @IsDisagreed= ISNULL(IsDisagreed,0)   from tblBSCAppraisalMaster  where FinancialYearId="+ FinancialYearId + @" and EmpInfoId=  " + id + @" 
SELECT   case when  ISNULL(@IsAgreed,0)=1 then 'Agreed with the final Appraisal'  when  ISNULL(@IsDisagreed,0)=1 then   'Disagreed with the final Appraisal'  else '' end  TinNo ,       ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin) ss, CAST((DATEDIFF(year, ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin), GETDATE())  - (CASE WHEN DATEADD(year, DATEDIFF(year, ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin), GETDATE()), ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin)) > GETDATE() THEN 1 ELSE 0 END)) AS NVARCHAR(max)) +' Years, ' +
...
CAST(DAY(GETDATE() - DATEADD(year, DATEDIFF(year, ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin), GETDATE()), ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin))) -(CASE WHEN DATEADD(year, DATEDIFF(year, ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin), GETDATE()), ISNULL(  ISNULL( dtEmployeePromotion.Effectivedate,dtEmployeePromotionHis.Effectivedate), EGI.DateOfJoin)) > GETDATE() THEN 0 ELSE 3 END)  AS NVARCHAR(max)) +' Days'  ServiceLength, sg.GradeCode+ isnull(' : '+sg.GradeName,'') GradeName, Stp.SalaryStepName, Sloc.SalaryLocation, R.EmpName AS ReportingToName,EGI.EmpInfoId,EGI.EmpName, EGI.DateOfJoin, CI.CompanyId, CI.CompanyName, DSN.DivisionId, DSN.DivisionName, DSNW.DivisionWId, DSNW.DivisionWingName,DPT.DepartmentId, DPT.DepartmentName,
    ...
    left JOIN (SELECT EmployeeId,MAX(EffectDate)AS Effectivedate  FROM  dbo.tblPromotionUpgrationHistory where TypeOfPromotion in ('Promotion','Upgradation') GROUP BY EmployeeId
        ...
        public DataTable GetEmployeeDetailsOKRREportMIDYear(int id, int FinancialYearId)
        {
            DataTable dt = GetEmployeeDetailsKPIREport(id, FinancialYearId);
            if (dt != null && dt.Columns.Contains("TinNo"))
            {
...
LEFT JOIN dbo.tblSuspendReasonEntry AS SPNDR ON DCPA.ReasonId =  SPNDR.SuspendReasonEntryId
        ...
        public DataTable GetAppraisalMasterIdFromAppraisalSelfMasterId(int id)
        {
            try
            {
                string query = @"select  * from dbo.tblBSCAppraisalMaster where BSCAppraisalSelfMasterId =" + id + "";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetAppraisalMasterIdFromAppraisalSelfMasterIdMIdYear(int id)
        {
            try
            {
...
inner join tblBSCAppraisalMaster app on appfin.BSCAppraisalMasterId=app.BSCAppraisalMasterId
...
where app.CurrentStatus='Approved' and app.EmpInfoId in (" + id + ") and fy.FinancialYearDesc='" + Finy+"'";
	   ...
	   CONVERT(NVARCHAR(11) , TrainingEnd , 106)TrainingEnd,*,Quater as quaterID, *
		 ...
		 LEFT JOIN dbo.tblBSCAppraisalMaster ON tblBSCAppraisalMaster.BSCAppraisalMasterId = tblAppraisalTrainingNeeds.BSCAppraisalMasterId
		 
		 LEFT JOIN dbo.tblEmpGeneralInfo Eg ON Eg.EmpInfoId = tblBSCAppraisalMaster.EmpinfoId where Eg.EmpMasterCode is not NULL 
 " + parm + "  ORDER BY Eg.EmpMasterCode";
            ...
            try
            {
                ...
                if (insert == true)
                {
                    ...
                    else
                    {
                        ...
                        string delQ = @"Delete from tblBSCAppraisalMaster where BSCAppraisalSelfMasterId = " + id + "";
        ...
        public int SaveAppraisalSelfMaster(BASAppraisalMaster aMaster, List<FuncData> FuncData, List<PartBData> PartBData, string Frm, string user)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = @"Insert into tblBSCAppraisalSelfMaster (EmpInfoId,FinancialYearId,EntryBy,EntryDate,ActionStatus,FYDes_BSCSelf ) values(@EmpInfoId,@FinancialYearId,@EntryBy,@EntryDate,@ActionStatus, (SELECT FinancialYearDesc 
                    FROM dbo.tblFinancialYear 
                    WHERE FinancialYearId =@FinancialYearId  ))";
     ...
     VALUES
        ...
        public bool SaveAppraialFunctionalDetails(List<AppraisalFunctionalAreaBSC> aList, int masterid, int selfMaster)
        {
            try
            {
                ...
                string query2 = @"delete from tblBSCAppraisalFuncArea where BSCAppraisalSelfMasterId = " + selfMaster + "";
                bool a = _aCommonInternalDal.DeleteDataByDeleteCommand(query2, DataBase.HRDB);
                ...
                foreach (var item in aList)
                {
                    ...
                    aParameters.Add(new SqlParameter("@SelfAssessment", (object)item.SelfAssessment ?? DBNull.Value));
                    aParameters.Add(new SqlParameter("@SupervisorAssessment", (object)item.SupervisorAssessment ?? DBNull.Value));
    ...
    //                }
                    ...
                    if (item.IsActive == true)
                    {
                        ...
                        string query = @"   INSERT INTO [dbo].[tblBSCAppraisalFuncArea]
           ...
           ,[KpiWeight]
           ,[Initiatives]
           ,[Deadline],SelfAssessment,SupervisorAssessment)
     VALUES
           (@BSCAppraisalMasterId
           ,@BSCAppraisalSelfMasterId
           ,@BSCAppraisalSelfFucAreaId
...
inner join (select   NewEmployeeId,OnlyViewComId from tblEmpSpecialTransfer where OnlyView=1) tblPer on reff.EmployeeId =tblPer.NewEmployeeId
        ...
        public DataTable GetAppraisalByKpiPermissionsssDashboard(string EmpID,   string param, string param22)
        {
            try
            {
                string query = @"SELECT Distinct * FROM(SELECT  ISNULL(app.BSCAppraisalSelfMasterId,0) BSCAppraisalSelfMasterId, e.EmpInfoId ,b.DeadLine, 'Set '+b.OptionInfo OptionInfo,  isnull(b.ExtensionDate,b.DeadLine) as ExtensionDate,
	...
	LEFT  JOIN (SELECT BSCAppraisalSelfMasterId,MAX(Version)MaxVer FROM dbo.tblBSCAppraisalSelfAppLog WHERE ActionStatus NOT IN
								...
								LEFT  JOIN dbo.tblBSCAppraisalSelfAppLog ON tblBSCAppraisalSelfAppLog.BSCAppraisalSelfMasterId = app.BSCAppraisalSelfMasterId
								LEFT JOIN dbo.tblEmpGeneralInfo ForEmp ON ForEmp.EmpInfoId=dbo.tblBSCAppraisalSelfAppLog.ForEmpInfoId
        WHERE b.EmpInfoId =  " + Convert.ToInt32(EmpID)   +param + "" +
                               "" +
                               "" +
                               @"	union all SELECT  ISNULL(app.BSCAppraisalSelfMasterId,0) BSCAppraisalSelfMasterId, e.EmpInfoId ,b.DeadLine, 'Set '+b.OptionInfo OptionInfo,  isnull(b.ExtensionDate,b.DeadLine) as ExtensionDate,
        ( 'Employee ID: '+ e.EmpMasterCode + ', Employee Name: ' + e.EmpName +    ISNULL(', Designation: ' + desg.Designation,'')) employee ,
...
inner join (select   NewEmployeeId,OnlyViewComId from tblEmpSpecialTransfer where OnlyView=1) tblPer on reff.EmployeeId =tblPer.NewEmployeeId
        ...
        WHERE reff.EmployeeId =  " + Convert.ToInt32(EmpID) +   param + param22 + " )HH";
...
LEFT JOIN tblDepartment dpt ON EMP.DepartmentId = dpt.DepartmentId
 INNER JOIN (SELECT EmpSkillWillMasterId,MAX(Version)MaxVer FROM dbo.tblEmpSkillWillAssessmentMasterAppLog WHERE ActionStatus NOT IN
    ...
    LEFT JOIN tblFinancialYear y ON A.FinancialYearId = y.FinancialYearId
        ...
        public DataTable GetAppraisalByKpiPermission2FinYearDashboard(string EmpID )
        {
            try
            {
                string query = @" SELECT * FROM 
...
left join tblBSCAppraisalMaster mas on mas.BSCAppraisalSelfMasterId=a.BSCAppraisalSelfMasterId  WHERE a.EmpInfoId = " + emp +
        ...
        public DataTable GetApprsaisalSelfByEmpFinYearMIDYear(int emp, int year)
        {
            return GetMidApprsaisalSelfByEmpFinYear(emp, year);
        }

        
        public DataTable GetMidApprsaisalSelfByEmpFinYear(int emp, int year)
        {
            try
            {
                string query = @"	SELECT A.BSCAppraisalSelfMasterId, isnull(mas.BSCAppraisalMasterId ,0)BSCAppraisalMasterId FROM dbo.tblBSCAppraisalSelfMaster a
left join tblMBSCAppraisalMaster mas on mas.BSCAppraisalSelfMasterId=a.BSCAppraisalSelfMasterId  WHERE a.EmpInfoId = " + emp +
        ...
        public DataTable GetCheckApprisalAlreadyExist(int BSCAppraisalSelfMasterId)
        {
            try
            {
                string query = @"	SELECT  BSCAppraisalSelfMasterId,BSCAppraisalMasterId FROM dbo.tblBSCAppraisalMaster   WHERE  BSCAppraisalSelfMasterId = " + BSCAppraisalSelfMasterId;
...
SELECT [BSCAppraisalMasterId]
      ...
      ,[CurrentStatus]
      ,[SelfApprove]
  FROM [dbo].[tblBSCAppraisalMaster] where BSCAppraisalMasterId=@BSCAppraisalMasterId
...
SELECT [AppraisalMasterAppLogId]
  ...
  FROM [dbo].tblAppraisalMasterAppLog where BSCAppraisalMasterId=@BSCAppraisalMasterId
	  ...
	  DELETE FROM [dbo].[tblBSCAppraisalMaster]
...
WHERE  A.BSCAppraisalSelfMasterId = " + id + "";
        ...
        public DataTable GetAppraisalSelfDetails(int id)
        {
            ...
            try
            {
                string query = @"SELECT '' SelfAssessment,'' SupervisorAssessment,  A.BSCAppraisalSelfFucAreaId AppraisalSelfFucAreaId ,
...
FROM    tblBSCAppraisalSelfFuncArea A 
inner join tblBSCOKRDimentionOrder ord on A.Dimension=ord.BSCOKRDimentionId
inner join tblBSCAppraisalFuncArea b on a.BSCAppraisalSelfFucAreaId=b.BSCAppraisalSelfFucAreaId
...
b.BSCAppraisalMasterId  =" + id + "  order by ord.OrderById asc";
        ...
        public DataTable GetAppraisalSelfFuncRpt(int id, bool _Option)
        {
            ...
            {
                string query = @"
SELECT  A.DimensionStr, A.DimensionStr BSCAppraisalSelfFucAreaId, '' AppraisalSelfMasterId ,   A.DimensionStr AppraisalSelfFucAreaId ,
	  ...
	  A.SupervisorMark SupervisorMark,'" + _Option + @"' IsActive
FROM    tblBSCAppraisalSelfFuncArea A 
inner join tblBSCOKRDimentionOrder ord on A.Dimension=ord.BSCOKRDimentionId 
WHERE    
A.BSCAppraisalSelfMasterId=" + id + "  order by ord.OrderById asc";
        ...
        public DataTable GetAppraisalFuncRpt(int id, bool _Option)
        {
            ...
            {
                string query = @"
SELECT  SelfAssessment, SupervisorAssessment, b.DimensionStr,  A.DimensionStr AppraisalSelfFucAreaId , b.DimensionStr BSCAppraisalSelfFucAreaId, SupervisorAssessment AppraisalSelfMasterId , 
         b.ObjectiveGoal AppraisalMasterId ,
        A.KPIMeasure KpiInfo ,
        A.KpiWeight ,
         b.Dimension
...
b.BSCAppraisalMasterId  =" + id + "  order by ord.OrderById asc";
        ...
        public DataTable GetAppraisalFuncRptMIDYear(int id, bool _Option)
        {
            try
            {
                string query = @"
SELECT  CommentsFunc SelfAssessment,SupervisorComments SupervisorAssessment, b.DimensionStr,  A.DimensionStr AppraisalSelfFucAreaId , b.DimensionStr BSCAppraisalSelfFucAreaId, SupervisorComments AppraisalSelfMasterId , 
         b.ObjectiveGoal AppraisalMasterId ,
        A.KPIMeasure KpiInfo ,
        A.KpiWeight ,
         b.Dimension
...
WHERE ISNULL(A.IsActive,1)=1 and   A.BSCAppraisalSelfMasterId =  " + id + "  order by ord.OrderById asc";
    ...
    SELECT  '' SelfAssessment, OrderById, 
        '' AS KpiInfo,
        A.Initiatives,
        ...
        0 AS KpiWeightPer,
        CONVERT(NVARCHAR(11), A.Deadline, 106) AS Deadline,
    case when masMid.SelfApprove='Approved' and masMid.CurrentStatus='Approved' then       ISNULL(MA.MidYearStatus,'') else '' end MidYearStatus,
        case when masMid.SelfApprove='Approved' and masMid.CurrentStatus='Approved' then   ISNULL(MA.SupervisorMark,0) else null end AS ResultYearEnd,
        A.SelfMark,
        A.Target,
        A.TargetPer,
...
left join tblMBSCAppraisalFuncArea MA on A.BSCAppraisalSelfFucAreaId=MA.BSCAppraisalSelfFucAreaId
left join tblMBSCAppraisalMaster masMid on MA.BSCAppraisalMasterId=masMid.BSCAppraisalMasterId   

    WHERE ISNULL(A.IsActive, 1) = 1
        AND A.BSCAppraisalSelfMasterId =  " + id + @" 
)
SELECT   SelfAssessment,DimensionStr DimensionStrHdn,ObjectiveGoal  ObjectiveGoalHdn,
...
inner join tblBSCOKRDimentionOrder ord on A.Dimension=ord.BSCOKRDimentionId
inner join tblBSCAppraisalFuncArea b on a.BSCAppraisalSelfFucAreaId=b.BSCAppraisalSelfFucAreaId
inner join tblBSCAppraisalMaster appMas on appMas.BSCAppraisalSelfMasterId=A.BSCAppraisalSelfMasterId
WHERE ISNULL(b.IsActive,0)=1 and     A.BSCAppraisalSelfMasterId = " + id + "";
        ...
        public DataTable GetAppraisalfDetailsFromSupMidYear(int id)
        {
            try
            {
                string query = @"
...
inner join tblBSCOKRDimentionOrder ord on A.Dimension=ord.BSCOKRDimentionId
inner join tblMBSCAppraisalFuncArea b on a.BSCAppraisalSelfFucAreaId=b.BSCAppraisalSelfFucAreaId
inner join tblMBSCAppraisalMaster appMas on appMas.BSCAppraisalSelfMasterId=A.BSCAppraisalSelfMasterId
WHERE ISNULL(b.IsActive,0)=1 and     A.BSCAppraisalSelfMasterId = " + id + "";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
        ...
        public DataTable GetMidAppraisalfDetailsFromSup(int id)
        {
...
SELECT '' KpiInfo, A.Initiatives,  A.BSCAppraisalSelfFucAreaId AppraisalSelfFucAreaId ,
        ...
        A.TargetPer,
		ISNULL(b.SupervisorMark,0)  as  SupervisorMark,ISNULL(b.IsActive,0) IsActive
FROM    tblMBSCAppraisalFuncArea A
inner join tblBSCOKRDimentionOrder ord on A.Dimension=ord.BSCOKRDimentionId
inner join tblMBSCAppraisalFuncArea b on a.BSCAppraisalSelfFucAreaId=b.BSCAppraisalSelfFucAreaId
WHERE ISNULL(b.IsActive,0)=1 and     A.BSCAppraisalSelfMasterId = " + id + "";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
...
WITH CTE AS (
    SELECT SelfAssessment, SupervisorAssessment,
        '' AS KpiInfo,
        b.Initiatives,
        ...
        A.Target,
        A.TargetPer,
        ISNULL(b.SupervisorMark, 0) AS SupervisorMark,
    ...
    INNER JOIN tblBSCOKRDimentionOrder ord 
        ON A.Dimension = ord.BSCOKRDimentionId
    INNER JOIN tblBSCAppraisalFuncArea b 
        ON A.BSCAppraisalSelfFucAreaId = b.BSCAppraisalSelfFucAreaId
    WHERE ISNULL(b.IsActive, 0) = 1
        AND A.BSCAppraisalSelfMasterId = " + id + @"
)
SELECT   SelfAssessment, SupervisorAssessment, DimensionStr DimensionStrHdn,ObjectiveGoal  ObjectiveGoalHdn,
    CASE WHEN RowNum = 1 THEN DimensionStr ELSE '' END AS DimensionStr,
    CASE WHEN RowNum = 1 THEN ObjectiveGoal ELSE '' END AS ObjectiveGoal,
    KpiInfo,
    Initiatives,
...
WHERE ISNULL(b.IsActive,0)=1 and     A.BSCAppraisalSelfMasterId = " + id + "";
        ...
        public DataTable GetAppraisalSelfB(int id)
        {
            try
            {
                string query = @"  SELECT case when masMid.SelfApprove='Approved' and masMid.CurrentStatus='Approved' then    ISNULL(mA.SupervisorScore,0) else 0 end  MidYearSuperScore, 0 SelfScore,  A.*
FROM    tblBSCAppraisalSelfBehaveArea A with (nolock)
left join tblMBSCAppraisalBehaveArea mA on ma.BSCAppraisalSelfBehaveId=A.BSCAppraisalSelfBehaveId
left join tblMBSCAppraisalMaster masMid on MA.BSCAppraisalMasterId=masMid.BSCAppraisalMasterId and masMid.CurrentStatus='Approved'
WHERE  A.BSCAppraisalSelfMasterId =  " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
        ...
        public DataTable GetAppraisalSelfB_Rpt(int id)
        {
            try
            {
                string query = @"select  CAST(ISNULL(SupervisorScore,0) as nvarchar(max))  SupervisorScore,  CAST(ISNULL(SetScore,0) as nvarchar(max)) SetScore,  CAST(ISNULL(SetScore,0) as nvarchar(max)) Score,*, 0 as SelfScore from tblBSCAppraisalSelfBehaveArea where BSCAppraisalSelfMasterId = " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetAppraisalB_Rpt(int id)
        {
            try
            {
                string query = @"select  CAST(ISNULL(SupervisorScore,0) as nvarchar(max))  SupervisorScore, CAST(ISNULL(SetScore,0) as nvarchar(max)) SetScore,  CAST(ISNULL(SelfScore,0) as nvarchar(max)) Score,  ISNULL(SelfScore,0) as SelfScore,* from tblBSCAppraisalBehaveArea where BSCAppraisalMasterId = " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetAppraisalB_RptMIDYear(int id)
        {
            try
            {
                string query = @"select  CAST(ISNULL(SupervisorScore,0) as nvarchar(max))  SupervisorScore, CAST(ISNULL(SetScore,0) as nvarchar(max)) SetScore,  CAST(ISNULL(SelfScore,0) as nvarchar(max)) Score,  ISNULL(SelfScore,0) as SelfScore,* from tblMBSCAppraisalBehaveArea where BSCAppraisalMasterId = " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
...
BSCAppraisalSelfMasterId=" + id + " ";
    ...
    ISNULL(tblBSCAppraisalBehaveArea.[BSCAppraisalSelfMasterId], 0) AS BSCAppraisalSelfMasterId,
    ISNULL([SupervisorScore], 0) AS SupervisorScore,
    ISNULL([SelfScore], 0) AS SelfScore,
    ISNULL([SetScore], 0) AS SetScore,
    *
FROM 
    tblBSCAppraisalBehaveArea 
	inner join tblBSCAppraisalMaster appMas on appMas.BSCAppraisalMasterId=tblBSCAppraisalBehaveArea.BSCAppraisalMasterId
 where tblBSCAppraisalBehaveArea.BSCAppraisalSelfMasterId =" + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetMidAppraisalPartB(int id)
        {
            try
            {
                string query = @"SELECT 
...
FROM 
 ...
 where BSCAppraisalSelfMasterId = " + id + " ";
        ...
        public DataTable GetAppraisalPartB_new(int id)
        {
            ...
            {
                string query = @"SELECT 
    ISNULL(main.[Score], 0) AS Score,
    ISNULL(main.[BSCAppraisalSelfMasterId], 0) AS BSCAppraisalSelfMasterId,
    ISNULL(main.[SupervisorScore], 0) AS SupervisorScore,
    ISNULL(main.[SelfScore], 0) AS SelfScore,
    ISNULL(main.[SetScore], 0) AS SetScore,
    ISNULL(main.[MidYearSuperScore], 0) AS MidYearSuperScore,
    *
FROM 
    tblBSCAppraisalBehaveArea main 
 where main.BSCAppraisalMasterId = " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
   ...
   from tblAppraisalSelfFuncArea A left join tblBSCAppraisalSelfMaster B on a.BSCAppraisalSelfMasterId = b.BSCAppraisalSelfMasterId
        ...
        public bool ApproveAppraisalOwnMark(int id)
        {
            try
            {
                ...
                string query = @"UPDATE dbo.tblBSCAppraisalMaster SET SelfApprove = 'Posted' WHERE BSCAppraisalMasterId = " +
                               id + "";
        ...
        public bool AppraisalRejectBySup(int user, int masterId, string entryBy, string remarks)
        {
            ...
            SELECT @reportTO = emp.ReportingEmpId  ,@appraisalSelf = A.BSCAppraisalSelfMasterId ,  @ActionVersion = ISNULL(a.ActionVersion,0) FROM dbo.tblBSCAppraisalMaster A LEFT JOIN dbo.tblEmpGeneralInfo emp ON a.EmpInfoId = emp.EmpInfoId  WHERE BSCAppraisalMasterId = @masterId
            
            IF(@reportTO = @user)
            BEGIN
            	UPDATE dbo.tblBSCAppraisalMaster SET CurrentStatus = 'Returned' , ActionVersion = @ActionVersion+1 , SelfApprove='Drafted'  ,   IsApprove = 0 , ApproveBy = @entryBy WHERE BSCAppraisalMasterId = @masterId
            ...
            ELSE 
            BEGIN 
            	UPDATE dbo.tblBSCAppraisalMaster SET CurrentStatus = 'Returned' , ActionVersion = @ActionVersion+1 ,    IsApprove = 0 , ApproveBy = @entryBy WHERE BSCAppraisalMasterId = @masterId
            	INSERT INTO dbo.tblAppraisalApproveLog
            	        ( BSCAppraisalSelfMasterId ,
            	          BSCAppraisalMasterId ,
            	          PreviousVersion ,
        ...
        public DataTable GetFinalMark(int id)
        {
            try
            {
                string query = @"SELECT *,tblbehav.SupScoreBehave+tblfuc.SupScoreFunc AS TotalScore FROM dbo.tblBSCAppraisalMaster
                                LEFT JOIN 
                                (SELECT BSCAppraisalMasterId,SUM(SupervisorMark)SupScoreFunc FROM dbo.tblBSCAppraisalFuncArea GROUP BY BSCAppraisalMasterId)AS tblfuc ON tblfuc.BSCAppraisalMasterId = tblBSCAppraisalMaster.BSCAppraisalMasterId
                                LEFT JOIN 
                                (SELECT BSCAppraisalMasterId,SUM(SupervisorScore)SupScoreBehave FROM dbo.tblBSCAppraisalBehaveArea GROUP BY BSCAppraisalMasterId) AS tblbehav ON tblbehav.BSCAppraisalMasterId = tblBSCAppraisalMaster.BSCAppraisalMasterId
                                WHERE tblBSCAppraisalMaster.BSCAppraisalMasterId='" + id+"'";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
        ...
        public bool SaveAppraisalMasterFromAppraisalSelf(string appselfMasterId)
        {
            try
            {
                ...
                string query = @"insert into tblBSCAppraisalMaster
...
select FinancialYearId,EmpInfoId,EntryDate,EntryBy,UpdateBy,UpdateDate,IsDelete,DeleteBy,IsApprove,ApproveBy,ApproveDate,BSCAppraisalSelfMasterId,'0',ActionStatus,'Posted',  (SELECT FinancialYearDesc 
       ...
       UPDATE dbo.tblBSCAppraisalMaster SET CurrentStatus = 'Approved' , ActionVersion = @ActionVersion+1 ,    IsApprove = 1 , ApproveBy =  @entryBy WHERE BSCAppraisalMasterId = @masterId
            	INSERT INTO dbo.tblAppraisalApproveLog
            	        ( BSCAppraisalSelfMasterId ,
            	          BSCAppraisalMasterId ,
            	          PreviousVersion ,
