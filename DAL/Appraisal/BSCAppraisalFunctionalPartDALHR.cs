...
{
    public class BSCAppraisalFunctionalPartDALHR
    {
...
where app.CurrentStatus='Approved' and app.EmpInfoId in (" + id + ") and fy.FinancialYearDesc='" + Finy+"'";
 ...
 " + parm + "  ORDER BY Eg.EmpMasterCode";
        ...
        public int SaveAppraisalMaster(AppraisalMaster aMaster, string user, string HR)
       {
           try
           {
               ...
               if (aMaster.BSCAppraisalMasterId > 0)
               {
                   pk = aMaster.BSCAppraisalMasterId;
                    ...
                    string delQuery = @"UPDATE [dbo].[tblBSCAppraisalMaster]
   SET [HRUpdateBy] = @EntryBy
      ,[HRUpdateDate] = getdate()
 WHERE BSCAppraisalMasterId=@BSCAppraisalMasterId ";
              ...
              // string query = @"Insert into tblBSCAppraisalMaster (EmpInfoId,FinancialYearId,EntryBy,EntryDate,BSCAppraisalSelfMasterId) values(@EmpInfoId,@FinancialYearId,@EntryBy,@EntryDate,@BSCAppraisalSelfMasterId)";
        ...
        public int SaveAppraisalSelfMasterMulti(AppraisalMaster aMaster, string user)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = @"Insert into tblBSCAppraisalSelfMaster (EmpInfoId,FinancialYearId,EntryBy,EntryDate,ActionStatus,IsFromGroupKPI, FYDes_BSCSelf ) values(@EmpInfoId,@FinancialYearId,@EntryBy,@EntryDate,@ActionStatus,1, (SELECT FinancialYearDesc 
        ...
        public int SaveAppraisalSelfMasterforSupper(AppraisalMaster aMaster, string user)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = @"Insert into tblBSCAppraisalSelfMaster (EmpInfoId,FinancialYearId,EntryBy,EntryDate,IsFunctionalArea,IsBehavioralArea,FYDes_BSCSelf ) values(@EmpInfoId,@FinancialYearId,@EntryBy,@EntryDate,@IsFunctionalArea,@IsBehavioralArea, (SELECT FinancialYearDesc 
                    FROM dbo.tblFinancialYear 
                    WHERE FinancialYearId =@FinancialYearId  )))";
...
INSERT INTO [dbo].[tblBSCAppraisalFuncAreaHR]
      ...
      select [BSCAppraisalMasterId]
           ...
           ,[Initiatives]
           ,[DimensionStr]
           ,[IsActive]  from tblBSCAppraisalFuncArea  where BSCAppraisalSelfMasterId= "  + selfMaster + @" 
delete from tblBSCAppraisalFuncArea where BSCAppraisalSelfMasterId = " + selfMaster + "";
                bool a = _aCommonInternalDal.DeleteDataByDeleteCommand(query2, DataBase.HRDB);
...
WHERE  A.BSCAppraisalSelfMasterId = " + id + "";
        ...
        public DataTable GetAppraisalSelfDetails(int id)
        {
            ...
            try
            {
                string query = @"SELECT  A.BSCAppraisalSelfFucAreaId AppraisalSelfFucAreaId ,
...
SELECT DimensionStr DimensionStrHdn,ObjectiveGoal  ObjectiveGoalHdn,
...
WHERE  A.BSCAppraisalSelfMasterId =  " + id + " ";
        ...
        public DataTable GetAppraisalB_Rpt(int id)
        {
            try
            {
                string query = @"select  CAST(ISNULL(SetScore,0) as int) SetScore,  CAST(ISNULL(SelfScore,0) as int) Score,  ISNULL(SelfScore,0) as SelfScore,* from tblBSCAppraisalBehaveArea where BSCAppraisalMasterId = " + id + " ";
                return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
            }
            catch (Exception ex)
            {
