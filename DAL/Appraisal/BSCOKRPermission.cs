 ...
 WHERE        reff.ShowCompany in (ComAssain)  and a.IsActive=1 "+param+") tbl";
        ...
        public int SaveKpiSetupMaster(KpiDeadlineMaster aMaster, string user)
        {
            try
            {
                ...
                else
                {
                    ...
                    string query = @"update tblBSCKpiDeadlineMaster set CompanyId = @CompanyId ,Subject=@Subject, FinancialYearId = @FinancialYearId , IsCommon = @IsCommon , UpdateBy = @UpdateBy , UpdateDate = @UpdateDate, DeclarationDate=@DeclarationDate where KPIDeadLineMasterId = @KPIDeadLineMasterId ";
        ...
        public bool SaveKpiSetupDetails(List<KPIDeadLineDetails> aDetails, int master)
        {
            try
            {
                ...
                bool result = false;
                string delQ = @"delete from tblBSCKPIDeadLineDetails where KPIDeadLineMasterId = " + master + "";
                ...
                foreach (KPIDeadLineDetails item in aDetails)
                {
                    ...
                    string query = @"insert into tblBSCKPIDeadLineDetails (KPIDeadLineMasterId, EmpinfoId, DeadLine, Remarks,OptionInfo) 
     ...
     VALUES
        ...
        {
            string query = @"
select distinct  OptionInfo  from  tblBSCKpiDeadlineMaster  a 
     left join (select count(EmpinfoId)TotalEmployee , KPIDeadLineMasterId, OptionInfo from tblBSCKPIDeadLineDetails group by KPIDeadLineMasterId,OptionInfo) B on a.KPIDeadLineMasterId = b.KPIDeadLineMasterId  
	 ...
	 where OptionInfo='" + OptionInfo+@"' and CompanyId =" +  CompanyId  + " and FinancialYearId =  " +  FinancialYearId ;
        ...
        public DataTable CheckPermisBSCKepiSetpExistss(string empInfoId, string FinancialYearId)
        {
            string query = @"select * from tblBSCKPIDeadLineDetails  d
inner join tblBSCKpiDeadlineMaster m on m.KPIDeadLineMasterId=d.KPIDeadLineMasterId
where EmpinfoId=" + empInfoId + " and m.FinancialYearId=" + FinancialYearId;
            return _aCommonInternalDal.DataContainerDataTable(query, DataBase.HRDB);
        }
...
left JOIN dbo.tblBSCKpiDeadlineMaster mas ON  mas.KPIDeadLineMasterId=dtls.KPIDeadLineMasterId
 ...
 where dtls.KPIDeadLineMasterId = " +
        ...
        public DataTable GetBSCKpiSetupDetailsByMaster(int id)
        {
            try
            {
                string query = @"SELECT OperationType OptionInfo, mas.FinancialYearId,   dtls.EmpinfoId,BSCOKRPermissionId KPIDeadLineDetailsId, dtls.KPIDeadLineMasterId, empInfo.EmpMasterCode, empInfo.EmpName,   '' DeadLine, '' Remarks, 0 AS DivisionName,
 desg.Designation, Dpt.DepartmentName,*  FROM   tblBSCOKRPermission dtls
INNER JOIN dbo.tblEmpGeneralInfo empInfo ON  dtls.EmpinfoId=empInfo.EmpInfoId
left JOIN dbo.tblDesignation desg ON  empInfo.DesignationId=desg.DesignationId
left JOIN dbo.tblDepartment Dpt ON  empInfo.DepartmentId=Dpt.DepartmentId
left JOIN dbo.tblBSCOKRPermissionMaster mas ON  mas.KPIDeadLineMasterId=dtls.KPIDeadLineMasterId
